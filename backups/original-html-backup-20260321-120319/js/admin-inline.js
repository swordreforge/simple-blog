function showToast(e, t = "success") {
    ...
    if (n) {
        ...
        switch (t) {
            case "success":
            default:
                o = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>';
                break;
            case "error":
                o = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>';
                break;
            case "warning":
                o = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>'
        }
...
async function fetchCategories() {
    try {
        ...
        var n = await (await fetch("/api/admin/categories", {
            headers: t
        })).json();
...
async function fetchTags() {
    try {
        ...
        e && (t.Authorization = "Bearer " + e);
        var n = await (await fetch("/api/admin/tags", {
            headers: t
        })).json();
...
async function populateCategorySelectorForNewArticle() {
    ...
    if (e) {
        ...
        if (e.innerHTML = "", 0 === t.length) {
            ...
            t.value = "", t.textContent = "暂无分类", e.appendChild(t)
        } else t.forEach(t => {
            var n = document.createElement("option");
...
async function fetchAdminData(e = 1, t = 10, n = 1, a = 10, o = 1, c = 10) {
    try {
        ...
        var i = await (await fetch(`/api/admin/users?page=${n}&limit=` + a, {
            headers: s
        })).json();
...
function getUsernameFromToken() {
    ...
    try {
        var t, n, a = e.split(".");
        return 3 !== a.length ? "管理员" : (t = a[1].replace(/-/g, "+").replace(/_/g, "/"), n = atob(t), JSON.parse(n).username || "管理员")
    } catch (e) {
...
function generateAdminIdenticon(e, t = 24) {
    ...
    let s = "";
    for (let e = 0; e < 5; e++)
        for (let t = 0; t < Math.ceil(2.5); t++) n >> 5 * e + t & 1 && (s = (s += `<rect x="${t}" y="${e}" width="1" height="1" fill="${c}"/>`) + `<rect x="${4-t}" y="${e}" width="1" height="1" fill="${c}"/>`);
    ...
    if (t) {
        ...
        if (n.clearRect(0, 0, t.width, t.height), e && 0 !== e.length) {
            const a = t.width - 100,
                ...
                d = (n.strokeStyle = "rgba(255, 255, 255, 0.3)", n.lineWidth = 1, n.beginPath(), n.moveTo(50, 50), n.lineTo(50, t.height - 50), n.lineTo(t.width - 50, t.height - 50), n.stroke(), a / (e.length - 1 || 1));
            n.beginPath(), n.strokeStyle = "rgba(255, 255, 255, 0.8)", n.lineWidth = 2, e.forEach((e, a) => {
                var s = 50 + a * d;
                e = t.height - 50 - e.count / c * o, 0 === a ? n.moveTo(s, e) : n.lineTo(s, e)
            }), n.stroke(), e.forEach((e, a) => {
                a = 50 + a * d;
                var s = t.height - 50 - e.count / c * o;
...
function initAnalytics() {
    var e;
    (e = document.getElementById("mostViewedLimit")) && e.addEventListener("change", loadMostViewedArticles), (e = document.getElementById("viewSourcesDays")) && e.addEventListener("change", loadViewSources), (e = document.getElementById("viewByCityDays")) && e.addEventListener("change", loadViewByCity), (e = document.getElementById("viewByIPDays")) && e.addEventListener("change", loadViewByIP), (e = document.getElementById("viewTrendDays")) && e.addEventListener("change", loadViewTrend), loadMostViewedArticles(), loadViewSources(), loadViewByCity(), loadViewByIP(), loadViewTrend()
}
...
uploadArea.addEventListener("click", () => {
    ...
    if (t) {
        const u = document.getElementById("uploadAuthor").value.trim();
        if (u) {
            const g = document.querySelector("#uploadCategorySelector .category-option.selected"),
                h = g ? g.dataset.categoryName : "",
                y = [],
                p = (document.querySelectorAll("#uploadTagSelector .tag-option.selected").forEach(e => {
                    y.push(e.dataset.tagName)
                }), this.querySelector('button[type="submit"]')),
                v = p.textContent;
            p.textContent = "上传中...", p.disabled = !0;
            try {
                const g = selectedFiles,
                    f = document.getElementById("uploadContent").value.trim();
                if (0 !== g.length || f) {
                    let b = f;
                    if (0 < g.length && !f) {
                        const e = g[0];
                        b = await readFileContent(e)
                    }
                    if (b && 0 !== b.trim().length) {
                        var n = document.getElementById("uploadStatus").value,
                            a = document.getElementById("uploadYear").value,
                            o = document.getElementById("uploadMonth").value,
                            c = document.getElementById("uploadDay").value;
                        let f = null;
                        if (a && o && c) {
                            const e = parseInt(a),
                                t = parseInt(o),
                                n = parseInt(c);
                            2020 <= e && e <= 2030 && 1 <= t && t <= 12 && 1 <= n && n <= 31 && (f = new Date(e, t - 1, n).toISOString())
                        }
                        var d = localStorage.getItem("auth_token"),
                            s = {
                                "Content-Type": "application/json"
                            },
                            r = (d && (s.Authorization = "Bearer " + d), {
                                title: t,
                                content: b,
                                author: u,
                                category: h,
                        ...
                        f && (r.created_at = f);
                        var i = await (await fetch("/api/admin/passages", {
                            method: "POST",
                            headers: s,
                        ...
                        if (i.success) {
                            ...
                            for (const a of g) try {
                                const e = new FormData,
                                    o = (e.append("file", a), e.append("passage_id", l), await fetch("/api/admin/attachments", {
                                        method: "POST",
                                        headers: {
                                    ...
                                    c = await o.json();
                                c.success ? t++ : (n++, m.push(a.name + ": " + (c.message || "未知错误")))
                            } catch (e) {
                                n++, m.push(a.name + ": " + (e.message || "网络错误"))
                            }
                            p.textContent = "创建成功!", p.style.background = "rgba(255, 183, 122, 0.8)";
                            let a = "文章创建成功！";
                            0 < t && (a += ` 成功上传 ${t} 个附件`), 0 < n && (a += ` 失败 ${n} 个附件`), setTimeout(() => {
                                closeModal("uploadModal"), p.textContent = v, p.disabled = !1, p.style.background = "rgba(255, 183, 122, 0.8)", this.reset(), uploadPreview.innerHTML = "", selectedFiles = [], fetchAdminData(), showToast(a, 0 < n ? "warning" : "success")
                            }, 2e3)
                        } else p.textContent = v, p.disabled = !1, showToast("创建失败：" + (i.message || "未知错误"), "error")
                    } else showToast("文章内容不能为空", "warning"), p.textContent = v, p.disabled = !1
                } else showToast("请选择要上传的文件或输入文章内容", "warning"), p.textContent = v, p.disabled = !1
            } catch (e) {
                console.error("创建文章失败:", e), p.textContent = v, p.disabled = !1, showToast("创建失败，无法连接到服务器，请检查网络连接后重试", "error")
            }
    ...
    if (t) {
        ...
        try {
            ...
            var a = await (await fetch("/api/admin/passages?id=" + t, {
                method: "PUT",
                headers: d,
                body: JSON.stringify({
            ...
            a.success ? (o.textContent = "保存成功!", o.style.background = "rgba(255, 183, 122, 0.8)", setTimeout(() => {
                closeModal("editModal"), o.textContent = c, o.disabled = !1, o.style.background = "rgba(255, 183, 122, 0.8)", fetchAdminData(), showToast("文章修改已保存！", "success")
            }, 2e3)) : (o.textContent = c, o.disabled = !1, showToast("保存失败：" + (a.message || "未知错误"), "error"))
        } catch (e) {
            console.error("保存文章失败:", e), o.textContent = c, o.disabled = !1, showToast("保存失败，请稍后重试", "error")
        }
    ...
    try {
        ...
        let s = "/api/admin/categories",
            r = "POST";
        t && (s += "?id=" + t, r = "PUT");
        var c = await (await fetch(s, {
            method: r,
            headers: d,
            body: JSON.stringify(o)
        })).json();
        c.success ? (n.textContent = "保存成功!", n.style.background = "rgba(255, 183, 122, 0.8)", setTimeout(() => {
        ...
        }, 2e3)) : (n.textContent = a, n.disabled = !1, showToast("保存失败：" + (c.message || "未知错误"), "error"))
    } catch (e) {
...
async function handleBatchDelete(e, t) {
    var n = t.split(",").map(e => parseInt(e.trim()));
    let a = "",
        o = "",
        c = null;
    switch (e) {
        case "batch-delete-comments":
            a = "/api/admin/comments/batch-delete", o = "批量删除评论成功", c = () => {
...
document.getElementById("confirmAction").addEventListener("click", async function() {
    if (currentAction && currentItemId)
        if (currentAction.startsWith("batch-delete-")) await handleBatchDelete(currentAction, currentItemId);
            ...
            try {
                let o = "";
                if ("delete" === currentAction) o = "/api/admin/passages?id=" + currentItemId;
                else if ("delete-comment" === currentAction) o = "/api/admin/comments/" + currentItemId;
                else if ("delete-user" === currentAction) o = "/api/admin/users/" + currentItemId;
                else if ("delete-category" === currentAction) o = "/api/admin/categories?id=" + currentItemId;
                else if ("delete-tag" === currentAction) o = "/api/admin/tags/" + currentItemId;
                else if ("delete-main-card" === currentAction) o = "/api/about/main-cards/delete?id=" + currentItemId;
                else if ("delete-sub-card" === currentAction) o = "/api/about/sub-cards/delete?id=" + currentItemId;
                else if ("delete-attachment" === currentAction) o = "/api/admin/attachments/" + currentItemId;
                else if ("batch-delete-attachment" === currentAction) {
                    var n = currentItemId.split(",");
                    let t = 0,
                        a = 0;
