...
window.loadCategoriesAndTags = async function loadCategoriesAndTags() {
...
window.fetchAdminData = async function fetchAdminData(e = 1, t = 10, n = 1, a = 10, o = 1, d = 10) {
    try {
        var c = localStorage.getItem("auth_token"),
            ...
            r = (i.success && i.data && 0 < i.data.length ? (updateArticlesTable(i.data), updateStatCard("totalArticles", i.pagination?.total || i.data.length), updatePagination(i.pagination)) : (showEmptyState("articlesTableBody", "暂无文章"), updateStatCard("totalArticles", 0), hidePagination()), await (await fetch(`/api/admin/users?page=${n}&limit=` + a, {
                headers: s
            })).json()),
    ...
    e && e.addEventListener("click", goToPrevPage), t && t.addEventListener("click", goToNextPage), n && n.addEventListener("click", () => {
        ...
        if (window.currentAction && window.currentItemId)
            if (window.currentAction.startsWith("batch-delete-")) await handleBatchDelete(window.currentAction, window.currentItemId);
                ...
                try {
                    let a = "";
                    if ("delete" === window.currentAction) a = "/api/admin/passages?id=" + window.currentItemId;
                    else if ("delete-comment" === window.currentAction) a = "/api/admin/comments/" + window.currentItemId;
                    else if ("delete-user" === window.currentAction) a = "/api/admin/users/" + window.currentItemId;
                    else if ("delete-category" === window.currentAction) a = "/api/admin/categories/" + window.currentItemId;
                    else if ("delete-tag" === window.currentAction) a = "/api/admin/tags/" + window.currentItemId;
                    else if ("delete-main-card" === window.currentAction) a = "/api/about/main-cards/delete?id=" + window.currentItemId;
                    else if ("delete-sub-card" === window.currentAction) a = "/api/about/sub-cards/delete?id=" + window.currentItemId;
                    else if ("delete-attachment" === window.currentAction) a = "/api/admin/attachments/" + window.currentItemId;
                    ...
                    n.success ? (closeModal("confirmModal"), showToast("删除成功！", "success"), ("delete-category" === window.currentAction || "delete-tag" === window.currentAction ? loadCategoriesAndTags : "delete-main-card" === window.currentAction ? (loadMainCards(), loadSubCards) : "delete-sub-card" === window.currentAction ? loadSubCards : "delete-attachment" === window.currentAction ? (selectedAttachments.delete(window.currentItemId), updateBatchActions(), loadAttachments) : fetchAdminData)()) : (console.error("DELETE failed", n), showToast("删除失败：" + (n.message || "未知错误"), "error"))
                } catch (e) {
...
function bindActionButtons() {
    document.querySelectorAll(".action-buttons button").forEach(e => {
        ...
        e.parentNode.replaceChild(t, e), t.addEventListener("click", async function() {
            ...
            if ("delete" === e || "delete-comment" === e || "delete-user" === e || "delete-category" === e || "delete-tag" === e || "delete-attachment" === e) {
                window.currentAction = e, window.currentItemId = t;
                let n = "";
                n = "delete" === e ? `确定要删除文章 #${t} 吗？此操作不可撤销。` : "delete-comment" === e ? `确定要删除评论 #${t} 吗？此操作不可撤销。` : "delete-user" === e ? `确定要删除用户 #${t} 吗？此操作不可撤销。` : "delete-category" === e ? `确定要删除分类 #${t} 吗？此操作不可撤销。` : "delete-tag" === e ? `确定要删除标签 #${t} 吗？此操作不可撤销。` : "delete-attachment" === e ? `确定要删除附件 #${t} 吗？此操作不可撤销。` : `确定要删除 #${t} 吗？此操作不可撤销。`, document.getElementById("confirmMessage").textContent = n, openModal("confirmModal")
            } else if ("edit" === e) try {
                    const e = localStorage.getItem("auth_token"),
                        o = {
                            "Content-Type": "application/json"
            ...
            } else if ("delete-tag" === e) window.currentAction = e, window.currentItemId = t, document.getElementById("confirmMessage").textContent = `确定要删除标签 #${t} 吗？此操作不可撤销。`, openModal("confirmModal");
                ...
                e && (n.Authorization = "Bearer " + e);
                var s = await (await fetch("/api/admin/tags/" + t, {
                    headers: n
                })).json();
                ...
                e && (n.Authorization = "Bearer " + e);
                var i = await (await fetch("/api/admin/categories/" + t, {
                    headers: n
                })).json();
                i.success && i.data ? (document.getElementById("categoryModalTitle").textContent = "编辑分类", document.getElementById("categoryName").value = i.data.name || "", document.getElementById("categoryDescription").value = i.data.description || "", document.getElementById("categoryIcon").value = i.data.icon || "", document.getElementById("categorySortOrder").value = i.data.sort_order || 0, document.getElementById("categoryEnabled").checked = i.data.is_enabled, document.getElementById("categoryForm").dataset.categoryId = t, openModal("categoryModal")) : showToast("获取分类详情失败：" + (i.message || "未知错误"), "error")
            } catch (e) {
...
async function handleBatchDelete(e, t) {
    ...
    switch (e) {
        ...
        case "batch-delete-attachment":
            a = "/api/admin/attachments/batch-delete", o = "批量删除附件成功", d = () => {
                        ...
                        if (c.success) {
                            ...
                            for (const c of s) try {
                                const e = new FormData,
                                    ...
                                    i = await s.json();
                                i.success ? n++ : (o++, d.push(c.name + ": " + (i.message || "未知错误")))
                            } catch (e) {
                                o++, d.push(c.name + ": " + (e.message || "网络错误"))
                            }
                            l.textContent = "创建成功!", l.style.background = "rgba(255, 183, 122, 0.8)";
                            let i = "文章创建成功！";
                            0 < n && (i += ` 成功上传 ${n} 个附件`), 0 < o && (i += ` 失败 ${o} 个附件`), setTimeout(() => {
                                closeModal("uploadModal"), l.textContent = m, l.disabled = !1, l.style.background = "rgba(255, 183, 122, 0.8)", this.reset(), uploadPreview.innerHTML = "", selectedFiles = [], fetchAdminData(), showToast(i, 0 < o ? "warning" : "success")
                            }, 2e3)
