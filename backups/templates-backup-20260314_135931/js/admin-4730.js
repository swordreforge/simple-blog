function showToast(t, a = "success") {
...
async function fetchAdminData(e = 1, t = 10, a = 1, n = 10, o = 1, d = 10) {
    try {
        var c = localStorage.getItem("auth_token"),
            ...
            i = (r.success && r.data && 0 < r.data.length ? (updateArticlesTable(r.data), updateStatCard("totalArticles", r.pagination?.total || r.data.length), updatePagination(r.pagination)) : (showEmptyState("articlesTableBody", "暂无文章"), updateStatCard("totalArticles", 0), hidePagination()), await (await fetch(`/api/admin/users?page=${a}&limit=` + n, {
                headers: s
            })).json()),
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    }), (a = document.getElementById("batchDeleteCommentsBtn")) && a.addEventListener("click", batchDeleteComments), e = document.getElementById("addCategoryBtn"), t = document.getElementById("refreshCategoriesBtn"), e && e.addEventListener("click", () => {
    ...
    }), t && t.addEventListener("click", loadCategoriesAndTags), (a = document.getElementById("batchDeleteCategoriesBtn")) && a.addEventListener("click", batchDeleteCategories), e = document.getElementById("addTagBtn"), t = document.getElementById("refreshTagsBtn"), a = document.getElementById("tagCategoryFilter"), e && e.addEventListener("click", async () => {
    ...
    document.querySelectorAll(".action-buttons button").forEach(e => {
        ...
        e.parentNode.replaceChild(t, e), t.addEventListener("click", async function() {
            ...
            if ("delete" === t || "delete-comment" === t || "delete-user" === t || "delete-category" === t) {
                ...
                var s = await (await fetch("/api/admin/tags?id=" + a, {
                    headers: g
                })).json();
                ...
                var r = await (await fetch("/api/admin/categories?id=" + a, {
                    headers: h
                })).json();
...
uploadArea.addEventListener("click", () => {
    ...
    if (s) {
        var r = document.getElementById("uploadAuthor").value.trim();
        if (r) {
    ...
    if (currentAction && currentItemId)
        if (currentAction.startsWith("batch-delete-")) await handleBatchDelete(currentAction, currentItemId);
            ...
            try {
                ...
                a.success ? (closeModal("confirmModal"), showToast("删除成功！", "success"), ("delete-category" === currentAction || "delete-tag" === currentAction ? loadCategoriesAndTags : "delete-main-card" === currentAction ? (loadMainCards(), loadSubCards) : "delete-sub-card" === currentAction ? loadSubCards : "delete-attachment" === currentAction ? (selectedAttachments.delete(currentItemId), updateBatchActions(), loadAttachments) : fetchAdminData)()) : showToast("删除失败：" + (a.message || "未知错误"), "error")
            } catch (n) {
