! function() {
    ...
    function t(e) {
        ...
        e.target.checked ? i.add(t) : (i.delete(t), m.selectAll.checked = !1), updateBatchActions()
    }
    ...
    function n() {
        document.body.insertAdjacentHTML("beforeend", '\n      <div class="modal active" id="uploadModal">\n        <div class="modal-content">\n          <div class="modal-header">\n            <h3>上传附件</h3>\n            <button class="modal-close" onclick="closeUploadModal()">×</button>\n          </div>\n          <div class="modal-body">\n            <div class="upload-area" id="uploadArea">\n              <div class="upload-icon"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg></div>\n              <div class="upload-text">\n                <h4>拖拽文件到这里或点击上传</h4>\n                <p>支持图片、文档、视频、音频、压缩包</p>\n              </div>\n              <input type="file" id="fileInput" multiple style="display: none;">\n            </div>\n            <div class="upload-preview" id="uploadPreview"></div>\n            <div class="form-group" style="margin-top: 20px;">\n              <label for="uploadPassageId">关联文章（可选）</label>\n              <select id="uploadPassageId" class="form-control">\n                <option value="">不关联</option>\n              </select>\n            </div>\n          </div>\n          <div class="btn-group" style="padding: 0 30px 30px;">\n            <button class="btn-secondary" onclick="closeUploadModal()">取消</button>\n            <button class="btn-primary" id="confirmUploadBtn" disabled>开始上传</button>\n          </div>\n        </div>\n      </div>\n    ');
        {
            ...
            const a = document.getElementById("uploadArea"),
                ...
                o = document.getElementById("uploadPreview"),
                l = document.getElementById("confirmUploadBtn"),
                d = document.getElementById("uploadPassageId");
            let i = [];

            function t(e) {
                i = Array.from(e), n(), l.disabled = 0 === i.length
            }

            function n() {
                o.innerHTML = i.map((e, t) => {
                    return `
        <div class="upload-item">
          ${e.type.startsWith("image/")?`<img src="${URL.createObjectURL(e)}" alt="${e.name}">`:`<div style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: #f5f5f5;">
              ${p((n=e.type,n.startsWith("image/")?"image":n.startsWith("video/")?"video":n.startsWith("audio/")?"audio":n.includes("pdf")||n.includes("document")||n.includes("word")||n.includes("excel")||n.includes("powerpoint")||!(n.includes("zip")||n.includes("tar")||n.includes("rar")||n.includes("7z"))?"document":"archive"))}
             </div>`}
      ...
      `;
            ...
            })(d), a.addEventListener("click", () => e.click()), e.addEventListener("change", e => {
                t(e.target.files)
                ...
                e.preventDefault(), a.classList.remove("dragover"), t(e.dataTransfer.files)
                ...
                if (0 !== i.length) {
                    ...
                    for (const n of i) {
                        ...
                        try {
                            const a = await fetch("/api/admin/attachments", {
                                    method: "POST",
                                    body: t
                                }),
                                l = await a.json();
    ...
    function l() {
        i.clear(), m.selectAll.checked = !1, updateBatchActions(), h()
    }
    ...
    function e() {
        ...
        }), m.batchDeleteBtn && m.batchDeleteBtn.addEventListener("click", a), m.batchSetPublicBtn && m.batchSetPublicBtn.addEventListener("click", () => o("public")), m.batchSetPrivateBtn && m.batchSetPrivateBtn.addEventListener("click", () => o("private")), m.cancelSelectionBtn && m.cancelSelectionBtn.addEventListener("click", l), loadAttachments(), fetch("/api/settings/appearance").then(e => e.json()).then(t => {
            function e() {
    ...
    window.loadAttachments = async function() {
        try {
            const t = new URLSearchParams({
                ...
                o = m.passageFilter.value,
                l = m.searchInput.value.trim();
            n && t.append("file_type", n), a && t.append("visibility", a), o && t.append("passage_id", o);
            ...
            if (e.success) {
                ...
                var i = Math.ceil(r / s);
                if (i <= 1) m.paginationContainer.style.display = "none";
                else {
                    m.paginationContainer.style.display = "flex", m.paginationInfo.textContent = `显示 ${(c-1)*s+1}-${Math.min(c*s,r)} 条，共 ${r} 条`, m.prevPageBtn.disabled = 1 === c, m.nextPageBtn.disabled = c === i;
                    let t = "",
                        n = Math.max(1, c - Math.floor(2.5)),
                        a = Math.min(i, n + 5 - 1);
                    for (let e = a - n < 4 ? Math.max(1, a - 5 + 1) : n; e <= a; e++) t += `<button class="pagination-page ${e===c?"active":""}" 
