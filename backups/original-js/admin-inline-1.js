async function fetchCategories() {
    try {
        var e = localStorage.getItem("auth_token"),
            ...
            a = (e && (t.Authorization = "Bearer " + e), await (await fetch("/api/admin/categories", {
                headers: t
            })).json());
...
async function populateCategorySelectorForNewArticle() {
    ...
    var e, t;
    a && (e = await fetchCategories(), a.innerHTML = "", 0 === e.length ? ((t = document.createElement("option")).value = "", t.textContent = "暂无分类", a.appendChild(t)) : e.forEach(e => {
        var t = document.createElement("option");
        t.value = e.name, t.textContent = e.name, a.appendChild(t)
    }))
}
async function fetchAdminData(e = 1, t = 10, a = 1, n = 10, o = 1, d = 10) {
    try {
        var c = localStorage.getItem("auth_token"),
            s = {
                "Content-Type": "application/json"
            },
            i = (c && (s.Authorization = "Bearer " + c), await (await fetch(`/api/admin/passages?page=${e}&limit=` + t, {
                headers: s
            })).json()),
            r = (i.success && i.data && 0 < i.data.length ? (updateArticlesTable(i.data), updateStatCard("totalArticles", i.pagination?.total || i.data.length), updatePagination(i.pagination)) : (showEmptyState("articlesTableBody", "暂无文章"), updateStatCard("totalArticles", 0), hidePagination()), await (await fetch(`/api/admin/users?page=${a}&limit=` + n, {
                headers: s
            })).json()),
            l = (r.success && r.data && 0 < r.data.length ? (updateUsersTable(r.data), updateStatCard("totalUsers", r.pagination?.total || r.data.length), updateUserPagination(r.pagination)) : (showEmptyState("usersTableBody", "暂无用户", 7), updateStatCard("totalUsers", 0), hideUserPagination()), await (await fetch(`/api/admin/comments?page=${o}&limit=` + d, {
                headers: s
            })).json()),
...
function showEmptyState(e, t, a = 6) {
    var n;
    (e = document.querySelector("#" + e)) && (n = localStorage.getItem("auth_token"), e.innerHTML = `
    <tr>
      <td colspan="${a}" style="text-align: center; padding: 40px; color: #999;">
        <div style="font-size: 48px; margin-bottom: 10px;"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg></div>
        <div>${n?t:"请先登录以查看数据"}</div>
        ${n?"":'<div style="margin-top: 10px; font-size: 0.9em;"><a href="#loginModal" onclick="document.getElementById(\'loginBtn\').click(); return false;" style="color: #007bff; text-decoration: none;">点击登录</a></div>'}
      </td>
    </tr>
  `)
}
...
function getUsernameFromToken() {
    ...
    try {
        var t, a, n = e.split(".");
        return 3 === n.length && (t = n[1].replace(/-/g, "+").replace(/_/g, "/"), a = atob(t), JSON.parse(a).username) || "管理员"
    } catch (e) {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    a && a.addEventListener("click", goToPrevCommentsPage), e && e.addEventListener("click", goToNextCommentsPage), t && t.addEventListener("click", () => {
        fetchAdminData(currentPage, currentLimit, currentUserPage, currentUserLimit, currentCommentsPage, currentCommentsLimit)
    });
    (a = document.getElementById("batchDeleteCommentsBtn")) && a.addEventListener("click", batchDeleteComments), e = document.getElementById("addCategoryBtn"), t = document.getElementById("refreshCategoriesBtn");
    ...
    (a = document.getElementById("batchDeleteCategoriesBtn")) && a.addEventListener("click", batchDeleteCategories), e = document.getElementById("addTagBtn"), t = document.getElementById("refreshTagsBtn"), a = document.getElementById("tagCategoryFilter");
    ...
    }), t && t.addEventListener("click", loadCategoriesAndTags), (e = document.getElementById("batchDeleteTagsBtn")) && e.addEventListener("click", batchDeleteTags), a && a.addEventListener("change", async function() {
        ...
        if (window.currentAction && window.currentItemId)
            if (window.currentAction.startsWith("batch-delete-")) await handleBatchDelete(window.currentAction, window.currentItemId);
                ...
                try {
                    ...
                    if ("delete" === window.currentAction) e = "/api/admin/passages?id=" + window.currentItemId;
                    ...
                    else if ("delete-attachment" === window.currentAction) e = "/api/admin/attachments/" + window.currentItemId;
                    else if ("batch-delete-attachment" === window.currentAction) {
                        let e = window.currentItemId.split(","),
                            t = 0,
                            a = 0;
                        for (const o of e) try {
                            ...
                            console.error("删除失败:", n), a++
                        }
                        return closeModal("confirmModal"), 0 < t && showToast(`成功删除 ${t} 个附件`, "success"), 0 < a && showToast(a + " 个附件删除失败", "error"), selectedAttachments.clear(), updateBatchActions(), void loadAttachments()
                    }
                    var a = await (await fetch(e, {
                        method: "DELETE",
                        headers: t
                    })).json();
                    a.success ? (closeModal("confirmModal"), showToast("删除成功！", "success"), ("delete-category" === window.currentAction || "delete-tag" === window.currentAction ? loadCategoriesAndTags : "delete-main-card" === window.currentAction ? (loadMainCards(), loadSubCards) : "delete-sub-card" === window.currentAction ? loadSubCards : "delete-attachment" === window.currentAction ? (selectedAttachments.delete(window.currentItemId), updateBatchActions(), loadAttachments) : fetchAdminData)()) : (console.error("DELETE failed", a), showToast("删除失败：" + (a.message || "未知错误"), "error"))
                } catch (n) {
                    console.error("删除失败:", n), alert("删除失败，请稍后重试")
                }
            }
    })
});
...
function generateAdminIdenticon(e, t = 24) {
    ...
    return n = e ? e.charAt(0).toUpperCase() : "?", "data:image/svg+xml;base64," + btoa(unescape(encodeURIComponent(`
...
async function batchDeleteComments() {
    ...
    0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length ? (window.currentAction = "batch-delete-comments", window.currentItemId = e.join(","), document.getElementById("confirmMessage").textContent = `确定要删除选中的 ${e.length} 条评论吗？此操作不可恢复！`, openModal("confirmModal")) : showToast("请选择要删除的评论", "warning")
}
...
async function batchDeleteCategories() {
    ...
    0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length ? (window.currentAction = "batch-delete-categories", window.currentItemId = e.join(","), document.getElementById("confirmMessage").textContent = `确定要删除选中的 ${e.length} 个分类吗？此操作不可恢复！`, openModal("confirmModal")) : showToast("请选择要删除的分类", "warning")
}
...
async function batchDeleteTags() {
    ...
    0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length ? (window.currentAction = "batch-delete-tags", window.currentItemId = e.join(","), document.getElementById("confirmMessage").textContent = `确定要删除选中的 ${e.length} 个标签吗？此操作不可恢复！`, openModal("confirmModal")) : showToast("请选择要删除的标签", "warning")
}
...
function updateArticlesTable(e) {
    const d = document.querySelector("#articles tbody");
    d && (d.innerHTML = "", e.forEach(e => {
        ...
        var n = "public" === (o = e.visibility || "public") ? "公开" : "私密",
            o = "public" === o ? "visibility-public" : "visibility-private";
        t.innerHTML = `
      <td>
      ...
      <td>${a}</td>
      <td><span class="visibility ${o}">${n}</span></td>
      <td class="action-buttons">
...
function bindBatchActionEvents() {
    ...
    e && e.addEventListener("change", t => {
        document.querySelectorAll(".article-checkbox").forEach(e => {
            e.checked = t.target.checked, (e = e.closest("tr")) && e.classList.toggle("selected", t.target.checked)
...
async function batchDeleteArticles() {
    ...
    0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length ? (window.currentAction = "batch-delete-articles", window.currentItemId = e.join(","), document.getElementById("confirmMessage").textContent = `确定要删除选中的 ${e.length} 篇文章吗？此操作不可恢复！`, openModal("confirmModal")) : showToast("请选择要删除的文章", "warning")
}
...
async function batchDeleteUsers() {
    ...
    0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length ? (window.currentAction = "batch-delete-users", window.currentItemId = e.join(","), document.getElementById("confirmMessage").textContent = `确定要删除选中的 ${e.length} 个用户吗？此操作不可恢复！`, openModal("confirmModal")) : showToast("请选择要删除的用户", "warning")
}
    ...
    document.querySelectorAll(".action-buttons button").forEach(e => {
        ...
        e.parentNode.replaceChild(t, e), t.addEventListener("click", async function() {
            ...
            if ("delete" === t || "delete-comment" === t || "delete-user" === t || "delete-category" === t || "delete-tag" === t || "delete-attachment" === t) {
                ...
                let e = "";
                e = "delete" === t ? `确定要删除文章 #${a} 吗？此操作不可撤销。` : "delete-comment" === t ? `确定要删除评论 #${a} 吗？此操作不可撤销。` : "delete-user" === t ? `确定要删除用户 #${a} 吗？此操作不可撤销。` : "delete-category" === t ? `确定要删除分类 #${a} 吗？此操作不可撤销。` : "delete-tag" === t ? `确定要删除标签 #${a} 吗？此操作不可撤销。` : "delete-attachment" === t ? `确定要删除附件 #${a} 吗？此操作不可撤销。` : `确定要删除 #${a} 吗？此操作不可撤销。`, document.getElementById("confirmMessage").textContent = e, openModal("confirmModal")
            } else if ("edit" === t) try {
                ...
                var i = await (await fetch("/api/admin/categories/" + a, {
                    headers: h
                })).json();
                i.success && i.data ? (document.getElementById("categoryModalTitle").textContent = "编辑分类", document.getElementById("categoryName").value = i.data.name || "", document.getElementById("categoryDescription").value = i.data.description || "", document.getElementById("categoryIcon").value = i.data.icon || "", document.getElementById("categorySortOrder").value = i.data.sort_order || 0, document.getElementById("categoryEnabled").checked = i.data.is_enabled, document.getElementById("categoryForm").dataset.categoryId = a, openModal("categoryModal")) : showToast("获取分类详情失败：" + (i.message || "未知错误"), "error")
            } catch (t) {
...
async function handleBatchDelete(e, t) {
    let a = t.split(",").map(e => parseInt(e.trim())),
        n = "",
        o = "",
        d = null;
    switch (e) {
        case "batch-delete-comments":
            n = "/api/admin/comments/batch-delete", o = "批量删除评论成功", d = () => {
...
}), document.getElementById("uploadForm").addEventListener("submit", async function(c) {
    c.preventDefault();
    const s = document.getElementById("uploadTitle").value.trim();
    if (s) {
        var i = document.getElementById("uploadAuthor").value.trim();
        if (i) {
            const e = document.querySelector("#uploadCategorySelector .category-option.selected"),
                g = e ? e.dataset.categoryName : "",
                h = [],
                y = (document.querySelectorAll("#uploadTagSelector .tag-option.selected").forEach(e => {
                    h.push(e.dataset.tagName)
                }), this.querySelector('button[type="submit"]')),
                p = y.textContent;
            y.textContent = "上传中...", y.disabled = !0;
            try {
                const c = selectedFiles,
                    t = document.getElementById("uploadContent").value.trim();
                if (0 !== c.length || t) {
                    let d = t;
                    if (0 < c.length && !t) {
                        const s = c[0];
                        d = await readFileContent(s)
                    }
                    if (d && 0 !== d.trim().length) {
                        let e = document.getElementById("uploadStatus").value,
                            t = document.getElementById("uploadYear").value,
                            a = document.getElementById("uploadMonth").value,
                            n = document.getElementById("uploadDay").value,
                            o = null;
                        if (t && a && n) {
                            const c = parseInt(t),
                                s = parseInt(a),
                                v = parseInt(n);
                            2020 <= c && c <= 2030 && 1 <= s && s <= 12 && 1 <= v && v <= 31 && (o = new Date(c, s - 1, v).toISOString())
                        }
                        var r = localStorage.getItem("auth_token"),
                            l = {
                                "Content-Type": "application/json"
                            },
                            m = (r && (l.Authorization = "Bearer " + r), {
                                title: s,
                                content: d,
                                author: i,
                                category: g,
                            ...
                            u = (o && (m.created_at = o), await (await fetch("/api/admin/passages", {
                                method: "POST",
                                headers: l,
                        ...
                        if (u.success) {
                            ...
                            for (const s of c) try {
                                const c = new FormData,
                                    f = (c.append("file", s), c.append("passage_id", e), await fetch("/api/admin/attachments", {
                                        method: "POST",
                                        headers: {
                                    ...
                                    b = await f.json();
                                b.success ? t++ : (a++, n.push(s.name + ": " + (b.message || "未知错误")))
                            } catch (c) {
                                a++, n.push(s.name + ": " + (c.message || "网络错误"))
                            }
                            y.textContent = "创建成功!", y.style.background = "rgba(255, 183, 122, 0.8)";
                            let o = "文章创建成功！";
                            0 < t && (o += ` 成功上传 ${t} 个附件`), 0 < a && (o += ` 失败 ${a} 个附件`), setTimeout(() => {
                                closeModal("uploadModal"), y.textContent = p, y.disabled = !1, y.style.background = "rgba(255, 183, 122, 0.8)", this.reset(), uploadPreview.innerHTML = "", selectedFiles = [], fetchAdminData(), showToast(o, 0 < a ? "warning" : "success")
                            }, 2e3)
                        } else y.textContent = p, y.disabled = !1, showToast("创建失败：" + (u.message || "未知错误"), "error")
                    } else showToast("文章内容不能为空", "warning"), y.textContent = p, y.disabled = !1
                } else showToast("请选择要上传的文件或输入文章内容", "warning"), y.textContent = p, y.disabled = !1
            } catch (c) {
                console.error("创建文章失败:", c), y.textContent = p, y.disabled = !1, showToast("创建失败，无法连接到服务器，请检查网络连接后重试", "error")
            }
...
}), document.getElementById("editForm").addEventListener("submit", async function(a) {
    a.preventDefault();
    var n = this.dataset.articleId;
    if (n) {
        ...
        try {
            ...
            let e = document.getElementById("editIsScheduled").checked,
                t = null;
            if (e) {
                const a = document.getElementById("editPublishedAt").value;
                a && (t = new Date(a).toISOString())
            }
            var o = await (await fetch("/api/admin/passages?id=" + n, {
                method: "PUT",
                headers: s,
                body: JSON.stringify({
            ...
            o.success ? (d.textContent = "保存成功!", d.style.background = "rgba(255, 183, 122, 0.8)", setTimeout(() => {
                closeModal("editModal"), d.textContent = c, d.disabled = !1, d.style.background = "rgba(255, 183, 122, 0.8)", fetchAdminData(), showToast("文章修改已保存！", "success")
            }, 2e3)) : (d.textContent = c, d.disabled = !1, showToast("保存失败：" + (o.message || "未知错误"), "error"))
        } catch (a) {
            console.error("保存文章失败:", a), d.textContent = c, d.disabled = !1, showToast("保存失败，请稍后重试", "error")
        }
    ...
    if (t) {
        ...
        try {
            ...
            var a = {
                ...
                o = (n && (a.password = n), await (await fetch("/api/admin/users/" + t, {
                    method: "PATCH",
                    headers: s,
...
}), document.getElementById("categoryForm").addEventListener("submit", async function(n) {
    n.preventDefault();
    const o = this.dataset.categoryId,
        d = this.querySelector('button[type="submit"]'),
        c = d.textContent;
    d.textContent = "保存中...", d.disabled = !0;
    try {
        ...
        let e = {
            ...
            t = "/api/admin/categories",
            a = "POST";
        o && (t = "/api/admin/categories/" + o, a = "PUT");
        var s = await (await fetch(t, {
            method: a,
            headers: i,
            body: JSON.stringify(e)
        })).json();
        s.success ? (d.textContent = "保存成功!", d.style.background = "rgba(255, 183, 122, 0.8)", setTimeout(() => {
            closeModal("categoryModal"), d.textContent = c, d.disabled = !1, d.style.background = "rgba(255, 183, 122, 0.8)", this.reset(), delete this.dataset.categoryId, loadCategoriesAndTags(), showToast(o ? "分类修改已保存！" : "分类创建成功！", "success")
        }, 2e3)) : (d.textContent = c, d.disabled = !1, showToast("保存失败：" + (s.message || "未知错误"), "error"))
    } catch (n) {
        console.error("保存分类失败:", n), d.textContent = c, d.disabled = !1, showToast("保存失败，请稍后重试", "error")
    }
}), document.getElementById("tagForm").addEventListener("submit", async function(n) {
    n.preventDefault();
    const o = this.dataset.tagId,
        d = this.querySelector('button[type="submit"]'),
        c = d.textContent;
    d.textContent = "保存中...", d.disabled = !0;
    try {
        ...
        s.success ? (d.textContent = "保存成功!", d.style.background = "rgba(255, 183, 122, 0.8)", setTimeout(() => {
            closeModal("tagModal"), d.textContent = c, d.disabled = !1, d.style.background = "rgba(255, 183, 122, 0.8)", this.reset(), delete this.dataset.tagId, loadCategoriesAndTags(), showToast(o ? "标签修改已保存！" : "标签创建成功！", "success")
        }, 2e3)) : (d.textContent = c, d.disabled = !1, showToast("保存失败：" + (s.message || "未知错误"), "error"))
    } catch (n) {
        console.error("保存标签失败:", n), d.textContent = c, d.disabled = !1, showToast("保存失败，请稍后重试", "error")
    }
}), window.currentAction = window.currentAction || null, window.currentItemId = window.currentItemId || null, window.selectedAttachments = window.selectedAttachments || new Set, document.querySelectorAll(".modal").forEach(e => {
