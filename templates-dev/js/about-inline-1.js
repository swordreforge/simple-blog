async function loadAboutCards() {
  var t = document.getElementById('aboutContainer');
  try {
    var e = await fetch('/api/about/main-cards');
    if (!e.ok) throw new Error('获取卡片数据失败');
    var n = await e.json();
    t.innerHTML = '';
    for (const e of n) {
      var a = await renderMainCard(e);
      t.appendChild(a);
    }
    document
...
async function renderMainCard(t) {
  ...
  try {
    ...
    if (n.ok) {
      var a = await n.json();
      if (0 < a.length) {
        const n = renderSubCards(a, t.layout_type);
        e.innerHTML += n;
      }
    }
  } catch (t) {
  ...
  mainTitle =
    ...
    document.querySelectorAll('nav a[href^="#"]').forEach(t => {
      t.addEventListener('click', function (t) {
        ...
        e.startsWith('#') &&
          (t.preventDefault(), (t = document.querySelector(e))) &&
