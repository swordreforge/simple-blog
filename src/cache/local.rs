...
/// 每条记录的值携带实际内容和该条目的 TTL，用于 per-entry 过期策略
type CacheValue = (String, Duration);

/// 自定义过期策略：从值中读取每条记录的 TTL
struct PerEntryExpiry;

impl moka::Expiry<String, CacheValue> for PerEntryExpiry {
...
impl LocalCacheBackend {
    ...
    pub fn new(max_size: Option<usize>) -> Self {
        ...
        // 使用 Moka 的同步缓存，配合自定义 Expiry 支持 per-entry TTL
        let cache = Cache::builder()
            .max_capacity(max_capacity as u64)
            .expire_after(PerEntryExpiry) // 每条记录使用 set 时传入的 TTL
...
impl CacheBackend for LocalCacheBackend {
    async fn get(&self, key: &str) -> Option<String> {
        // Moka 自动处理过期，无需手动检查；从元组中提取实际值
        self.cache.get(key).map(|(v, _)| v)
    }

    async fn set(&self, key: &str, value: &str, ttl: Duration) -> Result<(), CacheError> {
        // 将值与 TTL 一起存储，由 PerEntryExpiry 策略按条目应用
        self.cache.insert(key.to_string(), (value.to_string(), ttl));
        Ok(())
    }
    ...
    async fn delete_pattern(&self, pattern: &str) -> Result<(), CacheError> {
        ...
        let _predicate_id = self
            .cache
            .invalidate_entries_if(move |key: &String, _value: &CacheValue| glob_pattern.matches(key))
