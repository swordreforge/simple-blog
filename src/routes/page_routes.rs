...
pub fn configure_page_routes(cfg: &mut web::ServiceConfig) {
    ...
    cfg.route(
        "/passage/{id}",
        web::get().to(page_handlers::passage_detail),
    );
    ...
    cfg.route(
        "/markdown-editor",
        web::get().to(page_handlers::markdown_editor),
    );

    // 键盘测试页面
    cfg.route(
        "/keyboard-test",
        web::get().to(page_handlers::keyboard_test),
    );
    ...
    cfg.route(
        "/admin/dyn-routing",
        web::get().to(page_handlers::dyn_routing),
    );

    // 状态页面
    cfg.route(
        "/status/{status}",
        web::get().to(page_handlers::status_page),
    );
