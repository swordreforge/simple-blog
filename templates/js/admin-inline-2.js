...
async function loadDynamicRoutesCount() {
  try {
    ...
    var n,
      o,
      a = await (await fetch('/api/admin/dynamic-routes', { headers: t })).json();
    a.success &&
      a.data &&
      ((n = a.data.total || (Array.isArray(a.data) ? a.data.length : 0)),
      (o = document.getElementById('dynamicRoutesCount'))) &&
      (o.textContent = n);
  } catch (e) {
  ...
  try {
    var e,
      ...
      o =
        ...
        await fetch('/api/settings/appearance', { headers: n }));
    o.ok
      ? ((e = await o.json()),
        (originalAppearanceSettings = { ...e }),
        (document.getElementById('backgroundImage').value = e.background_image),
        (document.getElementById('mobileBackgroundImage').value = e.mobile_background_image),
        ...
        updatePreview())
      : console.error('加载设置失败:', o.statusText);
  } catch (e) {
...
async function saveSettings() {
  try {
    var e,
      ...
      i = {};
    for (const e in a) {
      var d = originalAppearanceSettings[e],
        r = a[e];
      r !== d && (i[e] = r);
    }
    0 === Object.keys(i).length
      ...
      : (e = await fetch('/api/settings/appearance', {
        ...
        ? (showToast('设置保存成功！', 'success'),
          ...
          'dark_mode_enabled' in i && applyDarkMode(i.dark_mode_enabled),
          updatePreview())
        : ((t = await e.json()), showToast('保存失败：' + (t.error || '未知错误'), 'error'));
  } catch (e) {
...
function updatePreview() {
  var e,
    ...
    r,
    l,
    s = document.getElementById('previewBox');
  s &&
    ((l = document.getElementById('backgroundImage').value),
    (r = document.getElementById('mobileBackgroundImage').value),
    (e = document.getElementById('globalOpacity').value),
    (t = document.getElementById('backgroundSize').value),
    (n = document.getElementById('backgroundPosition').value),
    (o = document.getElementById('backgroundRepeat').value),
    (a = document.getElementById('backgroundAttachment').value),
    (i = document.getElementById('blurAmount').value),
    (d = document.getElementById('saturateAmount').value),
    (r = window.innerWidth <= 768 && r ? r : l),
    (s.style.backgroundImage = `url('${r}')`),
    (s.style.backgroundSize = t),
    (s.style.backgroundPosition = n),
    (s.style.backgroundRepeat = o),
    (s.style.backgroundAttachment = a),
    s.style.setProperty('--blur-amount', i),
    s.style.setProperty('--saturate-amount', d),
    s.style.setProperty('--global-opacity', e),
    ...
    (s = document.getElementById('preview-style')) && s.remove(),
    (l.id = 'preview-style'),
...
function parseRgba(e) {
  return (e = e.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([\d.]+))?\)/))
    ? { r: parseInt(e[1]), g: parseInt(e[2]), b: parseInt(e[3]), a: e[4] ? parseFloat(e[4]) : 1 }
    : null;
}
...
((window.currentAction = null),
  ...
  document.addEventListener('DOMContentLoaded', async function () {
    ...
    (n && n.addEventListener('click', saveSettings),
      (n = document.getElementById('saveTemplateSettingsBtn')) &&
        n.addEventListener('click', saveTemplateSettings),
      (n = document.getElementById('saveMusicSettingsBtn')) &&
        n.addEventListener('click', saveMusicSettings),
      initMusicDragDrop());
    ...
    (i &&
      ...
      (n = document.getElementById('resetSettingsBtn')) &&
        n.addEventListener('click', resetSettings),
      [
      ...
      [
        ...
        (e = document.getElementById(e)) &&
          (e.addEventListener('input', updatePreview), e.addEventListener('change', updatePreview));
      }),
      window.addEventListener('resize', updatePreview),
      (n = document.querySelector('[data-tab="settings"]')) &&
...
async function loadTemplateSettings() {
  try {
    var e,
      ...
      o =
        (t && (n.Authorization = 'Bearer ' + t),
        await fetch('/api/settings/template', { method: 'GET', headers: n }));
    o.ok
      ? ((e = await o.json()),
        ...
        (document.getElementById('passageSummarizeEnabled').checked =
          e.passage_summarize_enabled || !1),
...
async function saveTemplateSettings() {
  try {
    var e,
      ...
      i = {};
    for (const e in a) a[e] !== originalTemplateSettings[e] && (i[e] = a[e]);
    0 === Object.keys(i).length
      ? showToast('没有检测到任何变化', 'warning')
      : (e = await fetch('/api/settings/template', {
        ...
        ? (showToast('模板设置保存成功！', 'success'), Object.assign(originalTemplateSettings, i))
        : ((t = await e.json()), showToast('保存失败：' + (t.error || '未知错误'), 'error'));
  } catch (e) {
...
async function loadMusicSettings() {
  try {
    var e,
      ...
      a =
        (n && (o.Authorization = 'Bearer ' + n),
        await fetch('/api/settings/music', { method: 'GET', headers: o }));
    a.ok
      ? ((e = await a.json()),
        ...
        (t = parseRgba(e.player_color || 'rgba(66, 133, 244, 0.9)')) &&
          (document.getElementById('musicPlayerColorPicker').value = rgbaToHex(t.r, t.g, t.b)))
...
async function saveMusicSettings() {
  try {
    var e,
      ...
      i = {};
    for (const e in a) a[e] !== originalMusicSettings[e] && (i[e] = a[e]);
    0 === Object.keys(i).length
      ? showToast('没有检测到任何变化', 'warning')
      : (e = await fetch('/api/settings/music', {
        ...
        ? (showToast('音乐设置保存成功！', 'success'), Object.assign(originalMusicSettings, i))
        : ((t = await e.json()), showToast('保存失败：' + (t.error || '未知错误'), 'error'));
  } catch (e) {
...
function initMusicDragDrop() {
  ...
  e &&
    ...
    e.addEventListener(
      'drop',
      t => {
        (e.classList.remove('dragover'), handleFileSelect(t.dataTransfer.files));
      },
      !1
    ),
    (o = document.getElementById('clearUploadBtn')) &&
      o.addEventListener('click', () => {
        isUploading
          ? showToast('正在上传中，请先取消上传', 'warning')
          : ((musicUploadQueue = []), updateUploadListUI());
      }),
    (o = document.getElementById('uploadAllMusicBtn')) &&
      o.addEventListener('click', startBatchUpload),
    (o = document.getElementById('cancelUploadBtn'))) &&
    o.addEventListener('click', cancelBatchUpload);
}
    ...
    (e = Array.from(e).filter(e => {
      ...
      return (
        (t =
          ...
          showToast('跳过不支持的文件: ' + e.name, 'warning'),
        t
      );
    })).length
      ? (e.forEach(e => {
...
function formatFileSize(e) {
  ...
  return 0 === e
    ...
    : ((t = Math.floor(Math.log(e) / Math.log(1024))),
      Math.round((e / Math.pow(1024, t)) * 100) / 100 + ' ' + ['B', 'KB', 'MB', 'GB'][t]);
}
...
function handleCoverSelect(e, t) {
  ...
  var o;
  n &&
    (['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'].includes(n.type)
      ? 5242880 < n.size
        ? (showToast('封面图片大小不能超过 5MB', 'error'), (t.value = ''))
        : (((o = new FileReader()).onload = function (t) {
            var o = musicUploadQueue.find(t => t.id == e);
            o && ((o.coverFile = n), (o.coverPreview = t.target.result), updateUploadListUI());
          }),
          o.readAsDataURL(n))
      : (showToast('请选择有效的图片文件（JPG、PNG、GIF、WebP）', 'error'), (t.value = '')));
}
function removeCover(e) {
  var t = musicUploadQueue.find(t => t.id == e);
  t &&
    ((t.coverFile = null),
    (t.coverPreview = null),
    (t = document.getElementById('coverInput-' + e)) && (t.value = ''),
    updateUploadListUI());
}
async function startBatchUpload() {
  if (isUploading) showToast('正在上传中...', 'info');
  else {
    var e = musicUploadQueue.filter(e => 'pending' === e.status);
    if (0 === e.length) showToast('没有等待上传的文件', 'warning');
    else {
      ((isUploading = !0), (uploadAbortController = new AbortController()));
      var t = document.getElementById('uploadAllMusicBtn'),
      ...
      ((isUploading = !1),
        ...
        t && (t.disabled = !1),
        n && (n.disabled = !0),
        0 < o && (showToast(`成功上传 ${o} 个文件`, 'success'), loadMusicPlaylist()),
        0 < a && showToast(a + ' 个文件上传失败', 'error'),
        setTimeout(() => {
...
function uploadSingleMusicFile(e, t) {
  return new Promise((n, o) => {
    var a = new FormData(),
      i = (a.append('file', e.file), e.file.name.replace(/\.[^/.]+$/, ''));
    (a.append('title', i),
      a.append('artist', '未知艺术家'),
      e.coverFile && a.append('cover', e.coverFile),
      (i = localStorage.getItem('auth_token')));
    const d = new XMLHttpRequest();
    (d.upload.addEventListener('progress', t => {
      t.lengthComputable &&
        ((t = Math.round((t.loaded / t.total) * 100)), (e.progress = t), updateUploadListUI());
    }),
      d.addEventListener('load', () => {
        if (200 <= d.status && d.status < 300) n();
        else
          try {
            var e = JSON.parse(d.responseText);
            o(new Error(e.error || '上传失败'));
          } catch (e) {
            o(new Error('上传失败: ' + d.status));
          }
      }),
      ...
      d.open('POST', '/api/music/upload'),
      i && d.setRequestHeader('Authorization', 'Bearer ' + i),
      d.send(a));
  });
}
function cancelBatchUpload() {
  ...
  isUploading &&
    ...
    (e = document.getElementById('uploadAllMusicBtn')),
...
async function loadMusicPlaylist() {
  try {
    var e = localStorage.getItem('auth_token'),
      ...
      n =
        (e && (t.Authorization = 'Bearer ' + e),
        await fetch('/api/music/playlist', { method: 'GET', headers: t }));
...
((window.deleteMusicTrack = async function (e) {
  if (confirm('确定要删除这首音乐吗？'))
    try {
      var t = localStorage.getItem('auth_token'),
        ...
        o =
          (t && (n.Authorization = 'Bearer ' + t),
          await fetch('/api/music/' + e, { method: 'DELETE', headers: n }));
  ...
  (window.changeMusicCover = async function (e) {
      ...
      (t.onchange = async t => {
        ...
        if (n)
          if (['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'].includes(n.type))
            if (5242880 < n.size) showToast('图片大小不能超过 5MB', 'error');
            else {
              var o = new FormData();
              o.append('cover', n);
              try {
                const t = localStorage.getItem('auth_token'),
                  n = {};
                t && (n.Authorization = 'Bearer ' + t);
                var a = await fetch(`/api/music/${e}/cover`, {
                  method: 'POST',
                  body: o,
                  headers: n,
                });
                if (a.ok)
                  (await a.json(), showToast('封面更新成功！', 'success'), loadMusicPlaylist());
                else {
                  const e = await a.json();
                  showToast('封面更新失败：' + (e.message || '未知错误'), 'error');
                }
              } catch (t) {
                (console.error('更新封面失败:', t), showToast('更新封面失败，请稍后重试', 'error'));
              }
            }
  ...
  (window.editMusicTitle = function (e, t) {
    const n = document.createElement('div'),
      o =
        ...
        n.querySelector('#musicTitleInput')),
      a = n.querySelector('#cancelBtn'),
      i = n.querySelector('#saveBtn'),
      d =
        ((a.onclick = () => {
        ...
        (i.onclick = async () => {
          ...
          if (t)
            try {
              ...
              var a = await fetch(`/api/music/${e}?action=title`, {
                method: 'PUT',
              ...
              (console.error('更新标题失败:', t), showToast('更新标题失败，请稍后重试', 'error'));
            }
        ...
        e => {
          'Escape' === e.key &&
            (document.body.removeChild(n), document.removeEventListener('keydown', d));
        });
...
function updateMainCardFilter() {
  var e,
    t = document.getElementById('subCardMainCardFilter');
  t &&
    ((e = t.value),
    (t.innerHTML =
...
async function addSubCard() {
  ...
  e
    ? ((document.getElementById('subCardModalTitle').textContent = '添加次卡片'),
      ...
      ((t = document.getElementById('subCardMainCardId')).innerHTML =
...
((window.loadMainCards = async function () {
  try {
    var e = localStorage.getItem('auth_token'),
      t = { 'Content-Type': 'application/json' },
      n =
        (e && (t.Authorization = 'Bearer ' + e),
        await fetch('/api/about/main-cards/admin', { method: 'GET', headers: t }));
    n.ok
  ...
  (window.loadSubCards = async function () {
    try {
      var e = localStorage.getItem('auth_token'),
        t = { 'Content-Type': 'application/json' },
        n =
          (e && (t.Authorization = 'Bearer ' + e),
          await fetch('/api/about/sub-cards/admin', { method: 'GET', headers: t }));
      n.ok
  ...
  (window.editMainCard = async function (e) {
    var t = mainCards.find(t => t.id === e);
    t &&
  ...
  (window.editSubCard = async function (e) {
    var t,
      n = subCards.find(t => t.id === e);
    n &&
      ((document.getElementById('subCardModalTitle').textContent = '编辑次卡片'),
      (document.getElementById('subCardId').value = n.id),
      ((t = document.getElementById('subCardMainCardId')).innerHTML =
        '<option value="">请选择主卡片</option>' +
        mainCards.map(e => `<option value="${e.id}">${e.title}</option>`).join('')),
      (t.value = n.main_card_id),
      (document.getElementById('subCardTitle').value = n.title || ''),
      (document.getElementById('subCardDescription').value = n.description || ''),
      (document.getElementById('subCardIcon').value = n.icon || ''),
      (document.getElementById('subCardLinkUrl').value = n.link_url || ''),
      (document.getElementById('subCardCustomCss').value = n.custom_css || ''),
      (document.getElementById('subCardSortOrder').value = n.sort_order || 0),
      (document.getElementById('subCardEnabled').checked = n.is_enabled),
      openModal('subCardModal'));
  }),
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    (e && e.addEventListener('click', addMainCard),
      (e = document.getElementById('addSubCardBtn')) && e.addEventListener('click', addSubCard),
      (e = document.getElementById('refreshAboutCardsBtn')) &&
        e.addEventListener('click', function () {
          (loadMainCards(), loadSubCards());
        }),
      (e = document.getElementById('subCardMainCardFilter')) &&
        e.addEventListener('change', updateSubCardsTable),
      (e = document.querySelector('[data-tab="about"]')) &&
        e.addEventListener('click', function () {
  ...
  subCardForm =
    (mainCardForm &&
      mainCardForm.addEventListener('submit', async function (e) {
        e.preventDefault();
        var t = document.getElementById('mainCardId').value,
          ...
          i = document.getElementById('mainCardCustomCss').value,
          d = parseInt(document.getElementById('mainCardSortOrder').value),
...
subCardForm &&
  subCardForm.addEventListener('submit', async function (e) {
    e.preventDefault();
    var t = document.getElementById('subCardId').value,
      ...
      i = document.getElementById('subCardIcon').value,
      d = document.getElementById('subCardLinkUrl').value,
...
async function loadFriendLinks() {
  try {
    var e,
      ...
      n = await fetch('/api/admin/friend-links?include_disabled=true', { headers: t });
    n.ok
      ? (e = await n.json()).success
...
function toggleFriendLinkSelection(e) {
  var t = selectedFriendLinkIds.indexOf(e);
  (-1 < t ? selectedFriendLinkIds.splice(t, 1) : selectedFriendLinkIds.push(e),
    updateBatchDeleteButton(),
    updateSelectAllCheckbox());
}
  ...
  if (0 !== selectedFriendLinkIds.length) {
    if (confirm(`确定要删除选中的 ${selectedFriendLinkIds.length} 个友链吗？\n此操作不可恢复！`))
      try {
        var e = localStorage.getItem('auth_token'),
          t = await (
            await fetch('/api/admin/friend-links/batch-delete', {
              method: 'POST',
        ...
        t.success
          ? (showToast(t.message || '批量删除成功', 'success'),
            (selectedFriendLinkIds = []),
            updateBatchDeleteButton(),
  ...
  if (o.nickname && o.link_url)
    try {
      const e = {
        ...
        i = n ? 'PUT' : 'POST',
        d = await fetch(a, { method: i, headers: e, body: JSON.stringify(o) }),
        r = await d.json();
      r.success
...
function initializeFriendLinksManagement() {
  ...
  (e && e.addEventListener('click', openAddFriendLinkModal),
    (e = document.getElementById('refreshFriendLinksBtn')) &&
      e.addEventListener('click', loadFriendLinks),
    (e = document.getElementById('selectAllFriendLinks')) &&
      e.addEventListener('change', toggleSelectAllFriendLinks),
    (e = document.getElementById('batchDeleteFriendLinksBtn')) &&
      e.addEventListener('click', batchDeleteFriendLinks),
    (e = document.getElementById('friendLinkForm')) &&
      e.addEventListener('submit', saveFriendLink));
}
((window.openEditFriendLinkModal = function (e) {
  var t = friendLinks.find(t => t.id === e);
  t
  ...
  (window.toggleFriendLinkStatus = async function (e) {
    var t = friendLinks.find(t => t.id === e);
    if (t)
      try {
        var n = {
          ...
          o = await (
            await fetch('/api/admin/friend-links/batch-update-status', {
              method: 'POST',
              headers: n,
        ...
        o.success
          ? (showToast(o.message || '状态更新成功', 'success'), await loadFriendLinks())
          : showToast(o.message || '状态更新失败', 'error');
      } catch (e) {
        (console.error('切换状态失败:', e), showToast('操作失败，请重试', 'error'));
      }
  }),
  (window.deleteFriendLink = function (e) {
    var t = friendLinks.find(t => t.id === e);
    t &&
      confirm(`确定要删除友链 "${t.nickname}" 吗？\n此操作不可恢复！`) &&
      ((t = localStorage.getItem('auth_token')),
...
const FileManager = {
    ...
    async loadSubDirectories(e, t) {
      try {
        var n = await (
          await fetch('/api/files?path=' + encodeURIComponent(e), {
    ...
    async loadFiles() {
      try {
        var e = await (
          await fetch('/api/files?path=' + encodeURIComponent(this.currentPath), {
            headers: { Authorization: this.getAuthHeader() },
          })
        ).json();
        e.success
          ? (this.renderFiles(e.data.files),
            this.updateBreadcrumb(e.data.current_path),
            this.updateBackButton(e.data.parent_path),
            this.updateFileCount(e.data.files.length))
    ...
    renderFiles(e) {
      ...
      0 === e.length
        ? ((t.innerHTML = ''), (n.style.display = 'flex'))
        : ((n.style.display = 'none'),
          (n = [...e].sort((e, t) =>
            e.is_dir && !t.is_dir ? -1 : !e.is_dir && t.is_dir ? 1 : e.name.localeCompare(t.name)
          )),
          (t.innerHTML = n.map(e => this.createFileItem(e)).join('')),
          t.querySelectorAll('.fm-file-item').forEach(e => {
            (e.addEventListener('click', t => {
              (t.stopPropagation(),
                (t = e.dataset.path),
                'true' === e.dataset.isDir ? this.navigateTo(t) : this.openFile(t));
            }),
              e.addEventListener('contextmenu', t => {
                t.preventDefault();
                var n = e.dataset.path,
    ...
    createFileItem(e) {
      ...
      t = e.is_dir
        ...
        : [
          ...
          ? ((n = `<img src="/${e.path}" alt="${e.name}" onerror="this.parentElement.innerHTML='<svg width=\\'16\\' height=\\'16\\' viewBox=\\'0 0 24 24\\' fill=\\'none\\' stroke=\\'currentColor\\' stroke-width=\\'2\\'><rect x=\\'3\\' y=\\'3\\' width=\\'18\\' height=\\'18\\' rx=\\'3\\' ry=\\'3\\'></rect><circle cx=\\'8.5\\' cy=\\'8.5\\' r=\\'1.5\\'></circle><path d=\\'21 15l-5-5L5 21\\'></path></svg>'">`),
            'image')
          : [
                '.mp3',
                '.flac',
            ...
            ? ((n = this.getFileIcon('audio')), 'audio')
            : [
                  '.mp4',
                  '.webm',
                  '.mkv',
              ...
              ? ((n = this.getFileIcon('video')), 'video')
              : '.md' === e.extension
                ? ((n = this.getFileIcon('markdown')), 'markdown')
                : '.pdf' === e.extension
                  ? ((n = this.getFileIcon('pdf')), 'pdf')
                  : ['.doc', '.docx', '.odt', '.rtf'].includes(e.extension)
                    ? ((n = this.getFileIcon('word')), 'word')
                    : ['.xls', '.xlsx', '.ods', '.csv'].includes(e.extension)
                      ? ((n = this.getFileIcon('excel')), 'excel')
                      : ['.ppt', '.pptx', '.odp'].includes(e.extension)
                        ? ((n = this.getFileIcon('ppt')), 'ppt')
                        : ['.zip', '.rar', '.7z', '.tar', '.gz', '.bz2', '.xz'].includes(
                              e.extension
                            )
                          ? ((n = this.getFileIcon('archive')), 'archive')
                          : [
                                '.html',
                                '.htm',
                                '.css',
                            ...
                            ? ((n = this.getFileIcon('code')), 'code')
                            : ['.txt', '.log'].includes(e.extension)
                              ? ((n = this.getFileIcon('text')), 'text')
                              : ['.ttf', '.otf', '.woff', '.woff2', '.eot'].includes(e.extension)
                                ? ((n = this.getFileIcon('font')), 'font')
                                : ['.db', '.sqlite', '.sqlite3', '.mdb', '.sql'].includes(
                                      e.extension
                                    )
                                  ? ((n = this.getFileIcon('database')), 'database')
                                  : [
                                        '.exe',
                                        '.app',
                                        '.dmg',
                                    ...
                                    ? ((n = this.getFileIcon('executable')), 'executable')
                                    : ((n = this.getFileIcon('default')), 'file');
      var o = this.formatFileSize(e.size);
      return `\n      <div class="fm-file-item ${t}" data-path="${e.path}" data-is-dir="${e.is_dir}">\n        <div class="fm-file-icon">${n}</div>\n        <div class="fm-file-name">${e.name}</div>\n        <div class="fm-file-meta">${e.is_dir ? '文件夹' : o}</div>\n      </div>\n    `;
    },
    getFileIcon: e =>
      ({
        image:
          '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="3" ry="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>',
        video:
          '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3" fill="currentColor"/><rect x="2" y="2" width="20" height="20" rx="3" ry="3"/></svg>',
        audio:
          '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3" fill="currentColor"/><circle cx="18" cy="16" r="3" fill="currentColor"/></svg>',
        document:
        ...
        markdown:
          '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="M10 13l-2 2 2 2"/><path d="M14 13l2 2-2 2"/></svg>',
        ...
        excel:
          '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><rect x="7" y="10" width="10" height="2"/><rect x="7" y="14" width="10" height="2"/><rect x="7" y="18" width="10" height="2"/></svg>',
        ...
        executable:
          '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><circle cx="12" cy="14" r="3"/><line x1="12" y1="11" x2="12" y2="17"/><line x1="9" y1="14" x2="15" y2="14"/></svg>',
      })[e] ||
    ...
    getParentPath(e) {
      return e === this.currentRoot
        ? null
        : ((e = e.split('/')).pop(), e.join('/') || this.currentRoot);
    },
    openFile(e) {
    ...
    showImagePreview(e) {
      ...
      ((n.src = e),
        (n.style.cssText =
          '\n      max-width: 90%;\n      max-height: 90%;\n      object-fit: contain;\n      border-radius: 8px;\n    '),
        ((e = document.createElement('div')).textContent = '点击关闭'),
        (e.style.cssText =
          '\n      position: absolute;\n      bottom: 30px;\n      color: white;\n      font-size: 16px;\n      opacity: 0.7;\n    '),
        t.appendChild(n),
        t.appendChild(e),
      ...
      const o = e => {
        'Escape' === e.key &&
          (document.body.removeChild(t), document.removeEventListener('keydown', o));
      };
    ...
    showContextMenu(e, t, n) {
      ...
      ((a.className = 'fm-context-menu'),
        ...
        (o = t.split('/').pop().split('.').pop().toLowerCase()),
        ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg'].includes(o),
        [
          {
            action: 'open',
            label: '打开',
          ...
          e.hide ||
            (((t = document.createElement('div')).className =
              'fm-context-menu-item' + (e.danger ? ' danger' : '')),
    ...
    openUploadModal() {
      ...
      ((e.type = 'file'),
        ...
        e.addEventListener('change', t => {
          (0 < (t = Array.from(t.target.files)).length && this.uploadFiles(t), e.remove());
        }),
    ...
    async createDirectory(e) {
      try {
        var t = await (
          await fetch('/api/files/create-dir', {
            method: 'POST',
    ...
    openDeleteModal() {
      var e;
      this.selectedFile &&
        ((e = this.selectedFile.path.split('/').pop()),
        confirm(`确定要删除 "${e}" 吗？此操作不可恢复！`)) &&
        this.deleteFile();
    },
    async deleteFile() {
      if (this.selectedFile)
        try {
          var e = await (
            await fetch('/api/files?path=' + encodeURIComponent(this.selectedFile.path), {
              method: 'DELETE',
              headers: { Authorization: this.getAuthHeader() },
            })
          ).json();
