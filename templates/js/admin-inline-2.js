...
function handleFileSelect(e) {
    e && 0 !== e.length && (0 !== (e = Array.from(e).filter(e => {
        ...
        musicUploadQueue.some(t => t.file.name === e.name && t.file.size === e.size) || musicUploadQueue.push({
            file: e,
...
function removeUploadItem(e) {
    isUploading ? showToast("正在上传中，请先取消上传", "warning") : (musicUploadQueue = musicUploadQueue.filter(t => t.id != e), updateUploadListUI())
}
...
function handleCoverSelect(e, t) {
    const n = t.files[0];
    var o;
    n && (["image/jpeg", "image/jpg", "image/png", "image/gif", "image/webp"].includes(n.type) ? 5242880 < n.size ? (showToast("封面图片大小不能超过 5MB", "error"), t.value = "") : ((o = new FileReader).onload = function(t) {
        var o = musicUploadQueue.find(t => t.id == e);
        o && (o.coverFile = n, o.coverPreview = t.target.result, updateUploadListUI())
    }, o.readAsDataURL(n)) : (showToast("请选择有效的图片文件（JPG、PNG、GIF、WebP）", "error"), t.value = ""))
}

function removeCover(e) {
    var t = musicUploadQueue.find(t => t.id == e);
    t && (t.coverFile = null, t.coverPreview = null, (t = document.getElementById("coverInput-" + e)) && (t.value = ""), updateUploadListUI())
}
    ...
    if (isUploading) showToast("正在上传中...", "info");
        ...
        if (0 === e.length) showToast("没有等待上传的文件", "warning");
            ...
            for (const t of e) {
                ...
                try {
                    await uploadSingleMusicFile(t, uploadAbortController.signal), t.status = "success", t.progress = 100, o++
                } catch (e) {
                    t.status = "error", t.error = e.message, a++, console.error(`上传文件 ${t.file.name} 失败:`, e)
                }
                updateUploadListUI()
            }
            isUploading = !1, uploadAbortController = null, t && (t.disabled = !1), n && (n.disabled = !0), 0 < o && (showToast(`成功上传 ${o} 个文件`, "success"), loadMusicPlaylist()), 0 < a && showToast(a + " 个文件上传失败", "error"), setTimeout(() => {
...
function uploadSingleMusicFile(e, t) {
    return new Promise((n, o) => {
        ...
        a.append("title", i), a.append("artist", "未知艺术家"), e.coverFile && a.append("cover", e.coverFile), i = localStorage.getItem("auth_token");
        const d = new XMLHttpRequest;
        d.upload.addEventListener("progress", t => {
            t.lengthComputable && (t = Math.round(t.loaded / t.total * 100), e.progress = t, updateUploadListUI())
            ...
            d.abort()
        }), d.open("POST", "/api/music/upload"), i && d.setRequestHeader("Authorization", "Bearer " + i), d.send(a)
    })
}
...
window.deleteMusicTrack = async function(e) {
    ...
    t.type = "file", t.accept = "image/jpeg,image/jpg,image/png,image/gif,image/webp", t.onchange = async t => {
            ...
            if (["image/jpeg", "image/jpg", "image/png", "image/gif", "image/webp"].includes(n.type))
                if (5242880 < n.size) showToast("图片大小不能超过 5MB", "error");
                    ...
                    try {
                        ...
                        if (a.ok) await a.json(), showToast("封面更新成功！", "success"), loadMusicPlaylist();
                        else {
    ...
    const n = document.createElement("div"),
        ...
        d = (a.onclick = () => {
            ...
            if (t) try {
                ...
                if (a.ok) await a.json(), showToast("标题更新成功！", "success"), document.body.removeChild(n), loadMusicPlaylist();
                else {
                    const e = await a.json();
            ...
            "Escape" === e.key && (document.body.removeChild(n), document.removeEventListener("keydown", d))
        });
    document.addEventListener("keydown", d)
};
let mainCards = [],
    subCards = [];
window.loadMainCards = async function loadMainCards() {
    try {
        var e = localStorage.getItem("auth_token"),
            t = {
                "Content-Type": "application/json"
...
window.loadSubCards = async function loadSubCards() {
    try {
        var e = localStorage.getItem("auth_token"),
            t = {
                "Content-Type": "application/json"
...
window.addMainCard = async function() {
    ...
    t && (document.getElementById("mainCardModalTitle").textContent = "编辑主卡片", document.getElementById("mainCardId").value = t.id, document.getElementById("mainCardTitle").value = t.title || "", document.getElementById("mainCardIcon").value = t.icon || "", document.getElementById("mainCardLayoutType").value = t.layout_type || "grid", document.getElementById("mainCardCustomCss").value = t.custom_css || "", document.getElementById("mainCardSortOrder").value = t.sort_order || 0, document.getElementById("mainCardEnabled").checked = t.is_enabled, openModal("mainCardModal"))
...
const mainCardForm = document.getElementById("mainCardForm"),
    subCardForm = (mainCardForm && mainCardForm.addEventListener("submit", async function(e) {
        e.preventDefault();
        ...
        try {
            ...
            let l = "/api/about/main-cards",
                s = "POST";
            t && (l = "/api/about/main-cards/update?id=" + t, s = "PUT"), (await fetch(l, {
                method: s,
                headers: e,
                body: JSON.stringify({
                    title: n,
            ...
            })).ok ? (showToast(t ? "更新成功" : "添加成功", "success"), closeModal("mainCardModal"), loadMainCards()) : showToast("操作失败", "error")
        } catch (e) {
...
subCardForm && subCardForm.addEventListener("submit", async function(e) {
    e.preventDefault();
    ...
    try {
        ...
        let c = "/api/about/sub-cards",
            u = "POST";
        t && (c = "/api/about/sub-cards/update?id=" + t, u = "PUT"), (await fetch(c, {
            method: u,
            headers: e,
            body: JSON.stringify({
                main_card_id: n,
        ...
        })).ok ? (showToast(t ? "更新成功" : "添加成功", "success"), closeModal("subCardModal"), loadSubCards()) : showToast("操作失败", "error")
    } catch (e) {
        console.error("操作失败:", e), showToast("操作失败", "error")
    }
});
...
function toggleSelectAllFriendLinks() {
    ...
    selectedFriendLinkIds = e.checked ? friendLinks.map(e => e.id) : [], document.querySelectorAll(".friend-link-checkbox").forEach(t => {
        t.checked = e.checked
...
window.openEditFriendLinkModal = function(e) {
    ...
    var t = friendLinks.find(t => t.id === e);
    t && confirm(`确定要删除友链 "${t.nickname}" 吗？\n此操作不可恢复！`) && (t = localStorage.getItem("auth_token"), fetch("/api/admin/friend-links/" + e, {
        method: "DELETE",
        headers: {
...
const FileManager = {
        ...
        renderFiles(e) {
            ...
            0 === e.length ? (t.innerHTML = "", n.style.display = "flex") : (n.style.display = "none", n = [...e].sort((e, t) => e.is_dir && !t.is_dir ? -1 : !e.is_dir && t.is_dir ? 1 : e.name.localeCompare(t.name)), t.innerHTML = n.map(e => this.createFileItem(e)).join(""), t.querySelectorAll(".fm-file-item").forEach(e => {
                e.addEventListener("click", t => {
                    t.stopPropagation(), t = e.dataset.path, "true" === e.dataset.isDir ? this.navigateTo(t) : this.openFile(t)
        ...
        createFileItem(e) {
            let t, n = this.getFileIcon("default");
            t = e.is_dir ? (n = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>', "directory") : [".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp", ".svg", ".ico", ".tiff", ".tif", ".avif", ".jxl"].includes(e.extension) ? (n = `<img src="/${e.path}" alt="${e.name}" onerror="this.parentElement.innerHTML='<svg width=\\'16\\' height=\\'16\\' viewBox=\\'0 0 24 24\\' fill=\\'none\\' stroke=\\'currentColor\\' stroke-width=\\'2\\'><rect x=\\'3\\' y=\\'3\\' width=\\'18\\' height=\\'18\\' rx=\\'3\\' ry=\\'3\\'></rect><circle cx=\\'8.5\\' cy=\\'8.5\\' r=\\'1.5\\'></circle><path d=\\'21 15l-5-5L5 21\\'></path></svg>'">`, "image") : [".mp3", ".flac", ".wav", ".ogg", ".m4a", ".aac", ".wma", ".opus", ".ape", ".wv", ".tta"].includes(e.extension) ? (n = this.getFileIcon("audio"), "audio") : [".mp4", ".webm", ".mkv", ".avi", ".mov", ".wmv", ".flv", ".m4v", ".3gp", ".ts", ".m2ts"].includes(e.extension) ? (n = this.getFileIcon("video"), "video") : ".md" === e.extension ? (n = this.getFileIcon("markdown"), "markdown") : ".pdf" === e.extension ? (n = this.getFileIcon("pdf"), "pdf") : [".doc", ".docx", ".odt", ".rtf"].includes(e.extension) ? (n = this.getFileIcon("word"), "word") : [".xls", ".xlsx", ".ods", ".csv"].includes(e.extension) ? (n = this.getFileIcon("excel"), "excel") : [".ppt", ".pptx", ".odp"].includes(e.extension) ? (n = this.getFileIcon("ppt"), "ppt") : [".zip", ".rar", ".7z", ".tar", ".gz", ".bz2", ".xz"].includes(e.extension) ? (n = this.getFileIcon("archive"), "archive") : [".html", ".htm", ".css", ".js", ".ts", ".jsx", ".tsx", ".vue", ".svelte", ".json", ".xml", ".yaml", ".yml", ".toml", ".ini", ".cfg", ".conf"].includes(e.extension) ? (n = this.getFileIcon("code"), "code") : [".txt", ".log"].includes(e.extension) ? (n = this.getFileIcon("text"), "text") : [".ttf", ".otf", ".woff", ".woff2", ".eot"].includes(e.extension) ? (n = this.getFileIcon("font"), "font") : [".db", ".sqlite", ".sqlite3", ".mdb", ".sql"].includes(e.extension) ? (n = this.getFileIcon("database"), "database") : [".exe", ".app", ".dmg", ".msi", ".deb", ".rpm", ".sh", ".bat", ".cmd", ".ps1"].includes(e.extension) ? (n = this.getFileIcon("executable"), "executable") : (n = this.getFileIcon("default"), "file");
            ...
            if (["jpg", "jpeg", "png", "gif", "webp", "bmp", "svg"].includes(t)) {
                ...
                if ("md" === t) {
                    ...
                    if (t.startsWith("/")) {
                        ...
                        if (-1 === n) return console.error("无效的 Markdown 路径:", e), void this.showToast("无效的 Markdown 路径", "error");
                        t = t.substring(n + 10)
                    } else {
                        if (!t.startsWith("markdown/")) return console.error("无效的 Markdown 路径:", e), void this.showToast("无效的 Markdown 路径", "error");
                        t = t.substring(9)
                    }
                    return t && "/" !== t && "" !== t.trim() ? void(window.MarkdownPreviewModal ? window.MarkdownPreviewModal.open(t) : console.error("MarkdownPreviewModal not available")) : (console.error("提取后的 Markdown 路径无效:", t), void this.showToast("无效的 Markdown 路径", "error"))
                }
                this.downloadFile(e)
            }
        },
        ...
        openUploadModal() {
            ...
            e.type = "file", e.multiple = !0, e.style.display = "none", e.addEventListener("change", t => {
