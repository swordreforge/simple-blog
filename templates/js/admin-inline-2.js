...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    var n = document.getElementById("saveSettingsBtn"),
        n = (n && n.addEventListener("click", saveSettings), document.getElementById("saveTemplateSettingsBtn")),
        n = (n && n.addEventListener("click", saveTemplateSettings), document.getElementById("saveMusicSettingsBtn"));
    n && n.addEventListener("click", saveMusicSettings), initMusicDragDrop();
    ...
    n = document.getElementById("resetSettingsBtn"), n && n.addEventListener("click", resetSettings), [{
        ...
        e && (e.addEventListener("input", updatePreview), e.addEventListener("change", updatePreview))
    }), window.addEventListener("resize", updatePreview), n = document.querySelector('[data-tab="settings"]');
...
function handleFileSelect(e) {
    e && 0 !== e.length && (0 !== (e = Array.from(e).filter(e => {
        var t = "." + e.name.split(".").pop().toLowerCase(),
            t = ["audio/mpeg", "audio/mp3", "audio/wav", "audio/wave", "audio/ogg", "audio/x-m4a", "audio/mp4"].includes(e.type) || [".mp3", ".wav", ".ogg", ".m4a"].includes(t);
        return t || showToast("跳过不支持的文件: " + e.name, "warning"), t
    })).length ? (e.forEach(t => {
...
function triggerCoverSelect(e) {
    e = document.getElementById("coverInput-" + e);
    e && e.click()
}
...
async function startBatchUpload() {
    if (isUploading) showToast("正在上传中...", "info");
        ...
        if (0 === n.length) showToast("没有等待上传的文件", "warning");
            ...
            const o = document.getElementById("uploadAllMusicBtn"),
            ...
            for (const o of n) {
                ...
                o.status = "uploading", updateUploadListUI();
                try {
                    await uploadSingleMusicFile(o, uploadAbortController.signal), o.status = "success", o.progress = 100, e++
                } catch (n) {
                    o.status = "error", o.error = n.message, t++, console.error(`上传文件 ${o.file.name} 失败:`, n)
                }
                updateUploadListUI()
            }
...
function updateSubCardsTable() {
    var t = document.getElementById("subCardsTableBody");
    if (t) {
        const n = document.getElementById("subCardMainCardFilter").value;
        let e = subCards;
        0 !== (e = n ? subCards.filter(e => e.main_card_id === parseInt(n)) : e).length ? t.innerHTML = e.map(t => (mainCards.find(e => e.id === t.main_card_id), `
      <tr>
        <td>${t.sort_order}</td>
        <td>${t.icon||""}</td>
        <td>${t.title}</td>
    ...
    `)).join("") : t.innerHTML = '<tr><td colspan="7" style="text-align: center; color: #999;">暂无次卡片</td></tr>'
    }
}
...
window.addMainCard = async function() {
    ...
    var e = document.getElementById("addMainCardBtn"),
        e = (e && e.addEventListener("click", addMainCard), document.getElementById("addSubCardBtn")),
        e = (e && e.addEventListener("click", addSubCard), document.getElementById("refreshAboutCardsBtn")),
        e = (e && e.addEventListener("click", function() {
            loadMainCards(), loadSubCards()
        }), document.getElementById("subCardMainCardFilter")),
        e = (e && e.addEventListener("change", updateSubCardsTable), document.querySelector('[data-tab="about"]'));
...
function initializeFriendLinksManagement() {
    var e = document.getElementById("addFriendLinkBtn"),
        e = (e && e.addEventListener("click", openAddFriendLinkModal), document.getElementById("refreshFriendLinksBtn")),
        e = (e && e.addEventListener("click", loadFriendLinks), document.getElementById("selectAllFriendLinks")),
        e = (e && e.addEventListener("change", toggleSelectAllFriendLinks), document.getElementById("batchDeleteFriendLinksBtn")),
        e = (e && e.addEventListener("click", batchDeleteFriendLinks), document.getElementById("friendLinkForm"));
    e && e.addEventListener("submit", saveFriendLink)
}
...
const FileManager = {
        ...
        async loadTree() {
            try {
                var e = ["img", "markdown", "attachments", "music"],
                    t = document.getElementById("fmTree");
                t.innerHTML = "";
                for (const o of e) {
                    var n = this.createTreeItem(o, !0);
                    t.appendChild(n), await this.loadSubDirectories(o, n)
                }
            } catch (e) {
        ...
        createTreeItem(e, t, n = null) {
            ...
            var a = document.createElement("span"),
                t = (a.className = "fm-tree-icon", a.innerHTML = t ? '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>' : '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path><path d="M12 11v6"></path><path d="M9 14l3 3 3-3"></path></svg>', o.appendChild(a), document.createElement("span"));
        ...
        renderFiles(e) {
            ...
            0 === e.length ? (t.innerHTML = "", n.style.display = "flex") : (n.style.display = "none", n = [...e].sort((e, t) => e.is_dir && !t.is_dir ? -1 : !e.is_dir && t.is_dir ? 1 : e.name.localeCompare(t.name)), t.innerHTML = n.map(e => this.createFileItem(e)).join(""), t.querySelectorAll(".fm-file-item").forEach(o => {
                o.addEventListener("click", e => {
                    e.stopPropagation();
                    e = o.dataset.path;
                    "true" === o.dataset.isDir ? this.navigateTo(e) : this.openFile(e)
    ...
    `
        ...
        async downloadFile(e) {
            try {
                ...
                if (t.ok) {
                    ...
                    void showToast(e.message || "下载失败", "error")
                }
            } catch (e) {
        ...
        showContextMenu(e, t, n) {
            ...
            o && o.remove();
            const a = document.createElement("div");
            a.className = "fm-context-menu", a.style.left = e.pageX + "px", a.style.top = e.pageY + "px";
        ...
        openUploadModal() {
            ...
            t.type = "file", t.multiple = !0, t.style.display = "none", t.addEventListener("change", e => {
                ...
                0 < e.length && this.uploadFiles(e), t.remove()
