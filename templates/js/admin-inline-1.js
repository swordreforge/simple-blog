  ...
  try {
    var e = localStorage.getItem('auth_token'),
      ...
      n =
        ...
        await (await fetch('/api/admin/categories', { headers: t })).json());
...
async function fetchTags(e = null) {
  try {
    ...
    var a = await (await fetch(o, { headers: n })).json();
...
async function populateCategorySelect(e, t = '') {
  ...
  var a;
  n &&
    ((e = await fetchCategories()),
    (n.innerHTML = ''),
    ((a = document.createElement('option')).value = ''),
    (a.textContent = '未分类'),
    ('' !== t && '未分类' !== t) || (a.selected = !0),
    n.appendChild(a),
    0 < e.length) &&
    e.forEach(e => {
...
async function populateTagSelector(e = []) {
  ...
  t &&
    ...
    (t.innerHTML = ''),
    0 === n.length
      ? (t.innerHTML = '<div style="color: #999; font-size: 0.9em;">暂无标签</div>')
      : (n.forEach(n => {
          var a;
          n.is_enabled &&
            (((a = document.createElement('div')).className = 'tag-option'),
  ...
  e &&
    ...
    0 !== t.length
      ? (t.forEach(t => {
          var n;
          t.is_enabled &&
            (((n = document.createElement('div')).className = 'tag-option'),
...
async function populateCategorySelectorForUpload() {
  ...
  var t;
  e &&
    ((t = await fetchCategories()),
    (e.innerHTML = ''),
    0 !== t.length
      ? t.forEach(t => {
          var n;
          t.is_enabled &&
            (((n = document.createElement('div')).className = 'category-option'),
...
((window.loadCategoriesAndTags = async function () {
  ...
  (window.fetchAdminData = async function (e = 1, t = 10, n = 1, a = 10, o = 1, d = 10) {
    try {
      var c = localStorage.getItem('auth_token'),
        s = { 'Content-Type': 'application/json' },
        i =
          (c && (s.Authorization = 'Bearer ' + c),
          await (await fetch(`/api/admin/passages?page=${e}&limit=` + t, { headers: s })).json()),
        r =
          (i.success && i.data && 0 < i.data.length
            ? (updateArticlesTable(i.data),
              updateStatCard('totalArticles', i.pagination?.total || i.data.length),
              updatePagination(i.pagination))
            : (showEmptyState('articlesTableBody', '暂无文章'),
              updateStatCard('totalArticles', 0),
              hidePagination()),
          await (await fetch(`/api/admin/users?page=${n}&limit=` + a, { headers: s })).json()),
        l =
          (r.success && r.data && 0 < r.data.length
            ? (updateUsersTable(r.data),
              updateStatCard('totalUsers', r.pagination?.total || r.data.length),
              updateUserPagination(r.pagination))
            : (showEmptyState('usersTableBody', '暂无用户', 7),
              updateStatCard('totalUsers', 0),
              hideUserPagination()),
          await (await fetch(`/api/admin/comments?page=${o}&limit=` + d, { headers: s })).json()),
        m =
          (l.success && l.data && 0 < l.data.length
            ? (updateCommentsTable(l.data), updateCommentsPagination(l.pagination))
            : (showEmptyState('commentsTableBody', '暂无评论', 6), hideCommentsPagination()),
          await (await fetch('/api/admin/stats', { headers: s })).json());
      if (m.success && m.data) {
        ...
        if (e && void 0 !== m.data.yesterday_visits) {
          ...
          'up' === n
            ...
            : 'down' === n
              ? ((e.className = 'stat-change negative'),
                (e.textContent = `较昨日 ${t.toFixed(1)}%`))
...
function updatePagination(e) {
  ...
  e
    ? ((t = parseInt(e.page) || 1),
      ...
      (e = parseInt(e.total) || 0),
      (currentPage = t),
      (currentLimit = n),
      (totalPages = Math.ceil(e / n)),
      (o = document.getElementById('paginationContainer')) && (o.style.display = 'flex'),
      (o = (t - 1) * n + 1),
      (n = Math.min(t * n, e)),
      (a = document.getElementById('paginationInfo')) &&
        (a.textContent = `显示 ${o}-${n} 条，共 ${e} 条`),
      (a = document.getElementById('prevPageBtn')),
      (o = document.getElementById('nextPageBtn')),
      a && (a.disabled = t <= 1),
      o && (o.disabled = t >= totalPages),
...
function generatePageButtons(e, t) {
  ...
  if (n) {
    ...
    (o - a < 4 && (1 === a ? (o = Math.min(t, 5)) : o === t && (a = Math.max(1, t - 4))),
      1 < a && (addPageButton(1, n), 2 < a) && addEllipsis(n));
...
function getUsernameFromToken() {
  ...
  try {
    ...
    return (
      (3 === a.length &&
        ((t = a[1].replace(/-/g, '+').replace(/_/g, '/')),
        (n = atob(t)),
        JSON.parse(n).username)) ||
      '管理员'
    );
  } catch (e) {
...
async function updateWelcomeMessage() {
  ...
  try {
    const e = localStorage.getItem('auth_token'),
      ...
      a =
        (e && (t.Authorization = 'Bearer ' + e),
        await fetch('/api/settings/template', { method: 'GET', headers: t }));
    if (a.ok) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (e && e.addEventListener('click', goToPrevPage),
    ...
    (e = document.getElementById('prevUserPageBtn')),
    (t = document.getElementById('nextUserPageBtn')),
    e && e.addEventListener('click', goToPrevUserPage),
    t && t.addEventListener('click', goToNextUserPage),
    (n = document.getElementById('prevCommentsPageBtn')),
    (e = document.getElementById('nextCommentsPageBtn')),
    (t = document.getElementById('refreshCommentsBtn')),
    n && n.addEventListener('click', goToPrevCommentsPage),
    e && e.addEventListener('click', goToNextCommentsPage),
    t &&
    ...
    (n = document.getElementById('batchDeleteCommentsBtn')) &&
      n.addEventListener('click', batchDeleteComments),
    (e = document.getElementById('addCategoryBtn')),
    ...
    t && t.addEventListener('click', loadCategoriesAndTags),
    (n = document.getElementById('batchDeleteCategoriesBtn')) &&
      n.addEventListener('click', batchDeleteCategories),
    (e = document.getElementById('addTagBtn')),
    (t = document.getElementById('refreshTagsBtn')),
    (n = document.getElementById('tagCategoryFilter')),
    e &&
    ...
    t && t.addEventListener('click', loadCategoriesAndTags),
    (e = document.getElementById('batchDeleteTagsBtn')) &&
      e.addEventListener('click', batchDeleteTags),
    n &&
    ...
    document.getElementById('confirmAction').addEventListener('click', async function () {
      if (window.currentAction && window.currentItemId)
        if (window.currentAction.startsWith('batch-delete-'))
          await handleBatchDelete(window.currentAction, window.currentItemId);
        else {
          var e = localStorage.getItem('auth_token'),
            t = { 'Content-Type': 'application/json' };
          e && (t.Authorization = 'Bearer ' + e);
          try {
            let a = '';
            if ('delete' === window.currentAction)
              a = '/api/admin/passages?id=' + window.currentItemId;
            else if ('delete-comment' === window.currentAction)
              a = '/api/admin/comments/' + window.currentItemId;
            else if ('delete-user' === window.currentAction)
              a = '/api/admin/users/' + window.currentItemId;
            else if ('delete-category' === window.currentAction)
              a = '/api/admin/categories/' + window.currentItemId;
            else if ('delete-tag' === window.currentAction)
              a = '/api/admin/tags/' + window.currentItemId;
            else if ('delete-main-card' === window.currentAction)
              a = '/api/about/main-cards/delete?id=' + window.currentItemId;
            else if ('delete-sub-card' === window.currentAction)
              a = '/api/about/sub-cards/delete?id=' + window.currentItemId;
            else if ('delete-attachment' === window.currentAction)
              a = '/api/admin/attachments/' + window.currentItemId;
            else if ('batch-delete-attachment' === window.currentAction) {
              let t = window.currentItemId.split(','),
                n = 0,
                a = 0;
              for (const o of t)
                try {
                  (await (await fetch('/api/admin/attachments/' + o, { method: 'DELETE' })).json())
                    .success
                    ? n++
                    : a++;
                } catch (e) {
              ...
              return (
                ...
                0 < n && showToast(`成功删除 ${n} 个附件`, 'success'),
                0 < a && showToast(a + ' 个附件删除失败', 'error'),
                selectedAttachments.clear(),
                updateBatchActions(),
                void loadAttachments()
              );
            }
            var n = await (await fetch(a, { method: 'DELETE', headers: t })).json();
            n.success
              ? (closeModal('confirmModal'),
                showToast('删除成功！', 'success'),
                ('delete-category' === window.currentAction || 'delete-tag' === window.currentAction
                  ? loadCategoriesAndTags
                  : 'delete-main-card' === window.currentAction
                    ? (loadMainCards(), loadSubCards)
                    : 'delete-sub-card' === window.currentAction
                      ? loadSubCards
                      : 'delete-attachment' === window.currentAction
...
function updateUserPagination(e) {
  ...
  e
    ? ((t = parseInt(e.page) || 1),
      ...
      (currentUserPage = t),
      (currentUserLimit = n),
      (totalUserPages = Math.ceil(e / n)),
      (o = document.getElementById('userPaginationContainer')) && (o.style.display = 'flex'),
      (o = (t - 1) * n + 1),
      (n = Math.min(t * n, e)),
      (a = document.getElementById('userPaginationInfo')) &&
        (a.textContent = `显示 ${o}-${n} 条，共 ${e} 条`),
      (a = document.getElementById('prevUserPageBtn')),
      (o = document.getElementById('nextUserPageBtn')),
      a && (a.disabled = t <= 1),
      o && (o.disabled = t >= totalUserPages),
...
function generateUserPageButtons(e, t) {
  ...
  if (n) {
    ...
    let a = Math.max(1, e - 2),
      o = Math.min(t, e + 2);
    (o - a < 4 && (1 === a ? (o = Math.min(t, 5)) : o === t && (a = Math.max(1, t - 4))),
      1 < a && (addUserPageButton(1, n), 2 < a) && addUserEllipsis(n));
...
function generateAdminIdenticon(e, t = 24) {
  ...
  for (let e = 0; e < 5; e++)
    for (let t = 0; t < Math.ceil(2.5); t++)
      (n >> (5 * e + t)) & 1 &&
        (s =
          (s += `<rect x="${t}" y="${e}" width="1" height="1" fill="${d}"/>`) +
          `<rect x="${4 - t}" y="${e}" width="1" height="1" fill="${d}"/>`);
  return (
    (a = e ? e.charAt(0).toUpperCase() : '?'),
    ...
    e.addEventListener('change', function () {
      (t.forEach(e => {
        ((e.checked = this.checked),
          (e = e.closest('tr')) &&
            (this.checked ? e.classList.add('selected') : e.classList.remove('selected')));
      }),
...
function updateSelectAllCheckbox() {
  var e,
    t = document.getElementById('selectAllComments'),
    n = document.querySelectorAll('.comment-checkbox');
  t &&
    0 < n.length &&
    ((e = Array.from(n).every(e => e.checked)),
    (n = Array.from(n).some(e => e.checked)),
    (t.checked = e),
    (t.indeterminate = n && !e));
}
...
async function batchDeleteComments() {
  ...
  0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length
    ? ((window.currentAction = 'batch-delete-comments'),
      (window.currentItemId = e.join(',')),
      (document.getElementById('confirmMessage').textContent =
        `确定要删除选中的 ${e.length} 条评论吗？此操作不可恢复！`),
...
function updateSelectAllCategoriesCheckbox() {
  var e,
    t = document.getElementById('selectAllCategories'),
...
async function batchDeleteCategories() {
  ...
  0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length
    ? ((window.currentAction = 'batch-delete-categories'),
      (window.currentItemId = e.join(',')),
      (document.getElementById('confirmMessage').textContent =
        `确定要删除选中的 ${e.length} 个分类吗？此操作不可恢复！`),
      openModal('confirmModal'))
...
function updateSelectAllTagsCheckbox() {
  var e,
    t = document.getElementById('selectAllTags'),
...
async function batchDeleteTags() {
  ...
  0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length
    ? ((window.currentAction = 'batch-delete-tags'),
      (window.currentItemId = e.join(',')),
      (document.getElementById('confirmMessage').textContent =
        `确定要删除选中的 ${e.length} 个标签吗？此操作不可恢复！`),
      openModal('confirmModal'))
...
function updateCommentsPagination(e) {
  ...
  e
    ? ((t = parseInt(e.page) || 1),
      (n = parseInt(e.limit) || 100),
      (e = parseInt(e.total) || 0),
      (currentCommentsPage = t),
      (currentCommentsLimit = n),
      (totalCommentsPages = Math.ceil(e / n)),
      (o = document.getElementById('commentsPaginationContainer')) && (o.style.display = 'flex'),
      (o = (t - 1) * n + 1),
      (n = Math.min(t * n, e)),
      (a = document.getElementById('commentsPaginationInfo')) &&
        (a.textContent = `显示 ${o}-${n} 条，共 ${e} 条`),
      (a = document.getElementById('prevCommentsPageBtn')),
      (o = document.getElementById('nextCommentsPageBtn')),
      a && (a.disabled = t <= 1),
      o && (o.disabled = t >= totalCommentsPages),
...
function generateCommentsPageButtons(e, t) {
  ...
  if (n) {
    ...
    let a = Math.max(1, e - 2),
      o = Math.min(t, e + 2);
    (o - a < 4 && (1 === a ? (o = Math.min(t, 5)) : o === t && (a = Math.max(1, t - 4))),
      1 < a && (addCommentsPageButton(1, n), 2 < a) && addCommentsEllipsis(n));
      ...
      ((n.innerHTML = `\n      <td>\n        <input type="checkbox" class="article-checkbox" data-id="${e.id}">\n      </td>\n      <td>${e.title}</td>\n      <td>管理员</td>\n      <td>${formatDate(e.created_at) || '2024-01-01'}</td>\n      <td>${a}</td>\n      <td><span class="visibility ${d}">${o}</span></td>\n      <td class="action-buttons">\n        <button class="btn btn-sm btn-view" data-action="view" data-id="${e.id}">查看</button>\n        <button class="btn btn-sm btn-edit" data-action="edit" data-id="${e.id}">编辑</button>\n        <button class="btn btn-sm btn-upload" data-action="upload" data-id="${e.id}">上传附件</button>\n        <button class="btn btn-sm btn-delete" data-action="delete" data-id="${e.id}">删除</button>\n      </td>\n    `),
        n.addEventListener('click', e => {
          e.target.closest('.action-buttons') ||
            ...
            (((e = n.querySelector('.article-checkbox')).checked = !e.checked),
            n.classList.toggle('selected', e.checked),
    ...
    e.addEventListener('change', e => {
      (document.querySelectorAll('.article-checkbox').forEach(t => {
        ((t.checked = e.target.checked),
          (t = t.closest('tr')) && t.classList.toggle('selected', e.target.checked));
      }),
        updateBatchActionsBar());
    }),
...
async function batchDeleteArticles() {
  ...
  0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length
    ? ((window.currentAction = 'batch-delete-articles'),
      (window.currentItemId = e.join(',')),
      (document.getElementById('confirmMessage').textContent =
        `确定要删除选中的 ${e.length} 篇文章吗？此操作不可恢复！`),
      openModal('confirmModal'))
...
async function batchDeleteUsers() {
  ...
  0 !== (e = Array.from(e).map(e => parseInt(e.dataset.id))).length
    ? ((window.currentAction = 'batch-delete-users'),
      (window.currentItemId = e.join(',')),
      (document.getElementById('confirmMessage').textContent =
        `确定要删除选中的 ${e.length} 个用户吗？此操作不可恢复！`),
      openModal('confirmModal'))
...
function formatDate(e) {
  return e
    ? new Date(e).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit' })
    : '';
}
      ...
      ((n.dataset.userId = e.id),
        ...
        n.addEventListener('click', e => {
          e.target.closest('.action-buttons') ||
            ...
            (e = n.querySelector('.user-checkbox')).disabled ||
            ((e.checked = !e.checked),
            n.classList.toggle('selected', e.checked),
            updateUserBatchActionsBar());
        }),
...
async function loadMostViewedArticles() {
  ...
  if (t) {
    ...
    try {
      var n = await (
        await fetch('/api/admin/analytics?action=most-viewed&limit=' + e, {
          headers: getAuthHeaders(),
        })
      ).json();
      n.success && n.data
        ? 0 === n.data.length
          ? (t.innerHTML = '<tr><td colspan="4" style="text-align: center;">暂无数据</td></tr>')
          : (t.innerHTML = n.data
              .map(
                (e, t) =>
                  `\n        <tr>\n          <td>${t + 1}</td>\n          <td>${e.title}</td>\n          <td>${e.author}</td>\n          <td>${e.view_count}</td>\n        </tr>\n      `
...
async function loadViewSources() {
  ...
  if (t) {
    ...
    try {
      var n = await (
        await fetch('/api/admin/analytics?action=view-sources&days=' + e, {
          headers: getAuthHeaders(),
        })
      ).json();
      n.success && n.data
        ? 0 === n.data.length
          ? (t.innerHTML = '<tr><td colspan="3" style="text-align: center;">暂无数据</td></tr>')
          : (t.innerHTML = n.data
              .map(
                (e, t) =>
...
async function loadViewByCity() {
  ...
  if (t) {
    ...
    try {
      var n = await (
        await fetch('/api/admin/analytics?action=view-by-city&days=' + e, {
          headers: getAuthHeaders(),
        })
      ).json();
      n.success && n.data
...
async function loadViewByIP() {
  ...
  if (t) {
    ...
    try {
      var n = await (
        await fetch('/api/admin/analytics?action=view-by-ip&days=' + e, {
          headers: getAuthHeaders(),
        })
      ).json();
      n.success && n.data
        ? 0 === n.data.length
          ? (t.innerHTML = '<tr><td colspan="8" style="text-align: center;">暂无数据</td></tr>')
          : (t.innerHTML = n.data
              .map(
                (e, t) =>
...
async function loadViewTrend() {
  ...
  if (document.getElementById('viewTrendChart'))
    try {
      var t = await (
        await fetch('/api/admin/analytics?action=view-trend&days=' + e, {
          headers: getAuthHeaders(),
        })
      ).json();
    ...
    if ((n.clearRect(0, 0, t.width, t.height), e && 0 !== e.length)) {
      const a = t.width - 100,
        ...
        c =
          ...
          n.beginPath(),
          n.moveTo(50, 50),
          n.lineTo(50, t.height - 50),
          n.lineTo(t.width - 50, t.height - 50),
          n.stroke(),
          a / (e.length - 1 || 1));
      (n.beginPath(),
        ...
        e.forEach((e, a) => {
          ...
          (n.beginPath(),
            ...
            n.arc(a, s, 4, 0, 2 * Math.PI),
            n.fill(),
            (n.fillStyle = 'rgba(255, 255, 255, 0.8)'),
            (n.font = '12px Arial'),
            (n.textAlign = 'center'),
            n.fillText(e.count, a, s - 10),
            (n.fillStyle = 'rgba(255, 255, 255, 0.6)'),
            (n.font = '10px Arial'),
            e.date &&
              'string' == typeof e.date &&
              n.fillText(e.date.substring(5), a, t.height - 50 + 15));
        }));
    } else
      ((n.font = '16px Arial'),
        (n.fillStyle = 'rgba(255, 255, 255, 0.7)'),
        (n.textAlign = 'center'),
        n.fillText('暂无数据', t.width / 2, t.height / 2));
  }
}
function initAnalytics() {
  var e = document.getElementById('mostViewedLimit');
  (e && e.addEventListener('change', loadMostViewedArticles),
    (e = document.getElementById('viewSourcesDays')) &&
      e.addEventListener('change', loadViewSources),
    (e = document.getElementById('viewByCityDays')) && e.addEventListener('change', loadViewByCity),
    (e = document.getElementById('viewByIPDays')) && e.addEventListener('change', loadViewByIP),
    (e = document.getElementById('viewTrendDays')) && e.addEventListener('change', loadViewTrend),
    ...
    (e.parentNode.replaceChild(t, e),
      t.addEventListener('click', async function () {
        ...
        if (
          ...
          ((window.currentAction = e), (window.currentItemId = t));
          var n =
            'delete' === e
              ? `确定要删除文章 #${t} 吗？此操作不可撤销。`
              : 'delete-comment' === e
                  ...
                  : 'delete-category' === e
                    ...
                    : 'delete-tag' === e
                      ...
                      : 'delete-attachment' === e
                        ? `确定要删除附件 #${t} 吗？此操作不可撤销。`
                        : `确定要删除 #${t} 吗？此操作不可撤销。`;
          ((document.getElementById('confirmMessage').textContent = n), openModal('confirmModal'));
        } else if ('edit' === e)
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var a = await (await fetch('/api/admin/passages?id=' + t, { headers: n })).json();
            if (a.success && a.data) {
              (await populateCategorySelect('editCategory', a.data.category || ''),
                (document.getElementById('editTitle').value = a.data.title || ''),
                (document.getElementById('editAuthor').value = a.data.author || '管理员'),
                (document.getElementById('editContent').value =
                  a.data.original_content || a.data.content || ''),
                (document.getElementById('editShowTitle').checked = !1 !== a.data.show_title),
                (document.getElementById('editCoverImage').value = a.data.cover_image || ''),
                (document.getElementById('editStatus').value = a.data.status || 'published'),
                (document.getElementById('editVisibility').value = a.data.visibility || 'public'));
              const e = a.data.is_scheduled || !1;
              document.getElementById('editIsScheduled').checked = e;
              var o = document.getElementById('editPublishedAtGroup');
              if (e) {
                if (((o.style.display = 'block'), a.data.published_at)) {
                  const e = new Date(a.data.published_at),
              ...
              (await populateTagSelector(n),
                (document.getElementById('editForm').dataset.articleId = t),
                openModal('editModal'));
            } else showToast('获取文章详情失败：' + (a.message || '未知错误'), 'error');
          } catch (e) {
          ...
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var d = await (await fetch('/api/admin/passages?id=' + t, { headers: n })).json();
            d.success && d.data
              ? ((document.getElementById('viewArticleId').textContent = '#' + d.data.id),
                (document.getElementById('viewArticleTitle').textContent = d.data.title || ''),
                ...
                (document.getElementById('viewArticleDate').textContent = d.data.created_at || ''),
                (document.getElementById('viewArticleContent').textContent = d.data.content || ''),
                openModal('viewArticleModal'))
              : showToast('获取文章详情失败：' + (d.message || '未知错误'), 'error');
          } catch (e) {
            (console.error('获取文章详情失败:', e),
              showToast('获取文章详情失败，请稍后重试', 'error'));
          }
          ...
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var c = await (await fetch('/api/admin/users/' + t, { headers: n })).json();
            if (c.success && c.data) {
              const e = c.data;
          ...
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var s = await (await fetch('/api/admin/users/' + t, { headers: n })).json();
            if (s.success && s.data) {
              const e = s.data;
            ...
            } else showToast('获取用户详情失败：' + (s.message || '未知错误'), 'error');
          } catch (e) {
            (console.error('获取用户详情失败:', e),
              showToast('获取用户详情失败，请稍后重试', 'error'));
          }
          ...
          ((window.currentAction = e),
            (window.currentItemId = t),
            (document.getElementById('confirmMessage').textContent =
              `确定要删除标签 #${t} 吗？此操作不可撤销。`),
            openModal('confirmModal'));
        else if ('edit-tag' === e)
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var i = await (await fetch('/api/admin/tags/' + t, { headers: n })).json();
            i.success && i.data
              ? ((document.getElementById('tagModalTitle').textContent = '编辑标签'),
                (document.getElementById('tagName').value = i.data.name || ''),
                (document.getElementById('tagDescription').value = i.data.description || ''),
                (document.getElementById('tagColor').value = i.data.color || '#007bff'),
                (document.getElementById('tagCategory').value = i.data.category_id || 0),
                (document.getElementById('tagSortOrder').value = i.data.sort_order || 0),
                (document.getElementById('tagEnabled').checked = i.data.is_enabled),
                (document.getElementById('tagForm').dataset.tagId = t),
                openModal('tagModal'))
              : showToast('获取标签详情失败：' + (i.message || '未知错误'), 'error');
          } catch (e) {
          ...
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var r = await (await fetch('/api/admin/categories/' + t, { headers: n })).json();
            r.success && r.data
              ? ((document.getElementById('categoryModalTitle').textContent = '编辑分类'),
                (document.getElementById('categoryName').value = r.data.name || ''),
                (document.getElementById('categoryDescription').value = r.data.description || ''),
                (document.getElementById('categoryIcon').value = r.data.icon || ''),
                (document.getElementById('categorySortOrder').value = r.data.sort_order || 0),
                (document.getElementById('categoryEnabled').checked = r.data.is_enabled),
                (document.getElementById('categoryForm').dataset.categoryId = t),
                openModal('categoryModal'))
              : showToast('获取分类详情失败：' + (r.message || '未知错误'), 'error');
          } catch (e) {
  ...
  uploadArea =
    ...
    document.getElementById('addUploadTagBtn').addEventListener('click', async function () {
      var e,
        t,
        n,
        a = document.getElementById('uploadTagInput'),
        o = a.value.trim();
      o
        ? ((e = document.getElementById('uploadTagSelector')),
          Array.from(e.querySelectorAll('.tag-option'))
            .map(e => (e.dataset.tagName || e.textContent.trim()).toLowerCase())
            .includes(o.toLowerCase())
            ? (showToast('标签已存在', 'warning'), (a.value = ''))
            : ((t = (t = [
                '#e74c3c',
                '#e67e22',
                '#f1c40f',
              ...
              ((n = document.createElement('div')).className = 'tag-option selected'),
              (n.dataset.tagName = o),
              (n.dataset.isNew = 'true'),
              ...
              e.appendChild(n),
              (a.value = ''),
              showToast(`标签 "${o}" 已添加`, 'success')))
        : showToast('请输入标签名称', 'warning');
    }),
    document.getElementById('addUploadCategoryBtn').addEventListener('click', async function () {
      var e,
        t = document.getElementById('uploadCategoryInput'),
        n = t.value.trim();
      if (n) {
        const a = document.getElementById('uploadCategorySelector');
        Array.from(a.querySelectorAll('.category-option'))
          .map(e => (e.dataset.categoryName || e.textContent.trim()).toLowerCase())
          .includes(n.toLowerCase())
          ? (showToast('分类已存在', 'warning'), (t.value = ''))
          : (((e = document.createElement('div')).className = 'category-option selected'),
            (e.dataset.categoryName = n),
...
function handleFiles(e) {
  for (let n = 0; n < e.length; n++) {
    ...
    var t;
    10485760 < a.size
      ? showToast(`文件 ${a.name} 超过10MB限制`, 'error')
...
async function handleBatchDelete(e, t) {
  ...
  switch (e) {
    ...
    case 'batch-delete-attachment':
      ((a = '/api/admin/attachments/batch-delete'),
        (o = '批量删除附件成功'),
        (d = () => {
  ...
  try {
    const e = await fetch(a, {
      ...
      t = await e.json();
    t.success
      ? (closeModal('confirmModal'), showToast(t.message || o, 'success'), d && d())
...
(uploadArea.addEventListener('click', () => {
  ...
  document.getElementById('uploadForm').addEventListener('submit', async function (e) {
    ...
    if (t) {
      ...
      if (n) {
        const r = document.querySelector('#uploadCategorySelector .category-option.selected'),
          l = r ? r.dataset.categoryName : '',
          m = [],
          u =
            (document.querySelectorAll('#uploadTagSelector .tag-option.selected').forEach(e => {
              m.push(e.dataset.tagName);
            }),
            this.querySelector('button[type="submit"]')),
          g = u.textContent;
        ((u.textContent = '上传中...'), (u.disabled = !0));
        try {
          var a = selectedFiles,
            o = document.getElementById('uploadContent').value.trim();
          if (0 !== a.length || o) {
            let r = o;
            if (0 < a.length && !o) {
              const e = a[0];
              r = await readFileContent(e);
            }
            if (r && 0 !== r.trim().length) {
              let o = document.getElementById('uploadStatus').value,
              ...
              if (h && y && p) {
                ...
                2020 <= e &&
                  ...
                  1 <= n &&
                  n <= 31 &&
                  (v = new Date(e, t - 1, n).toISOString());
              }
              var d = localStorage.getItem('auth_token'),
                c = { 'Content-Type': 'application/json' },
                s =
                  (d && (c.Authorization = 'Bearer ' + d),
                  { title: t, content: r, author: n, category: l, tags: m.join(','), status: o }),
                i =
                  (v && (s.created_at = v),
                  await (
                ...
                for (const s of a)
                  try {
                    const e = new FormData(),
                      ...
                      i = await a.json();
                    i.success ? n++ : (o++, c.push(s.name + ': ' + (i.message || '未知错误')));
                  } catch (e) {
                    (o++, c.push(s.name + ': ' + (e.message || '网络错误')));
                  }
                ((u.textContent = '创建成功!'), (u.style.background = 'rgba(255, 183, 122, 0.8)'));
                let s = '文章创建成功！';
                (0 < n && (s += ` 成功上传 ${n} 个附件`),
                  0 < o && (s += ` 失败 ${o} 个附件`),
                  setTimeout(() => {
                    (closeModal('uploadModal'),
                      (u.textContent = g),
                      (u.disabled = !1),
                      (u.style.background = 'rgba(255, 183, 122, 0.8)'),
                      this.reset(),
                      (uploadPreview.innerHTML = ''),
                      (selectedFiles = []),
                      fetchAdminData(),
                      showToast(s, 0 < o ? 'warning' : 'success'));
                  }, 2e3));
              } else
                ((u.textContent = g),
                  (u.disabled = !1),
                  showToast('创建失败：' + (i.message || '未知错误'), 'error'));
            } else
              (showToast('文章内容不能为空', 'warning'), (u.textContent = g), (u.disabled = !1));
          } else
        ...
        (showToast('请输入作者名称', 'warning'), document.getElementById('uploadAuthor').focus());
  ...
  document.getElementById('articleForm').addEventListener('submit', async function (e) {
    ...
    if (t) {
      ...
      try {
        ...
        n.success
          ? ((a.textContent = '保存成功!'),
            (a.style.background = 'rgba(255, 183, 122, 0.8)'),
            setTimeout(() => {
          ...
          : ((a.textContent = o),
            (a.disabled = !1),
            showToast('保存失败：' + (n.message || '未知错误'), 'error'));
      } catch (e) {
  ...
  document.getElementById('editForm').addEventListener('submit', async function (e) {
    ...
    if (t) {
      ...
      try {
        ...
        var n = await (
          await fetch('/api/admin/passages?id=' + t, {
            method: 'PUT',
            headers: d,
            body: JSON.stringify({
      ...
      if (((t.style.display = 'block'), !n.value)) {
        const e = new Date(),
          t =
            ...
            new Date(e.getTime() - 6e4 * e.getTimezoneOffset()).toISOString().slice(0, 16));
        n.value = t;
      }
  ...
  document.getElementById('userForm').addEventListener('submit', async function (e) {
    ...
    try {
      const e = await fetch('/api/register', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
          body: JSON.stringify({ username: a, email: o, password: d, role: c }),
        }),
        t = await e.json();
      t.success
  ...
  document.getElementById('editUserForm').addEventListener('submit', async function (e) {
    ...
    if (t) {
      ...
      try {
        ...
        o.success
          ? ((d.textContent = '保存成功!'),
            (d.style.background = 'rgba(255, 183, 122, 0.8)'),
            setTimeout(() => {
          ...
          : ((d.textContent = c),
            (d.disabled = !1),
            showToast('保存失败：' + (o.message || '未知错误'), 'error'));
      } catch (e) {
  ...
  document.getElementById('categoryForm').addEventListener('submit', async function (e) {
    ...
    try {
      ...
      let c = {
        ...
        i = 'POST';
      t && ((s = '/api/admin/categories/' + t), (i = 'PUT'));
      var o = await (await fetch(s, { method: i, headers: d, body: JSON.stringify(c) })).json();
      o.success
        ? ((n.textContent = '保存成功!'),
