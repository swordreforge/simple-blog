...
function updateCategoriesTable(e) {
  ...
  t &&
    ...
    0 !== e.length
      ...
      : (t.innerHTML =
          '\n      <tr>\n        <td colspan="7" style="text-align: center; padding: 40px; color: #999;">\n          <div style="margin-bottom: 10px;"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"></path></svg></div>\n          <div>暂无分类</div>\n        </td>\n      </tr>\n    '));
}
...
async function fetchTags() {
  try {
    var e = localStorage.getItem('auth_token'),
      ...
      n =
        (e && (t.Authorization = 'Bearer ' + e),
        await (await fetch('/api/admin/tags', { headers: t })).json());
    return n.success && n.data ? n.data : [];
  } catch (e) {
    return (console.error('获取标签列表失败:', e), []);
  }
}
    ...
    var c = localStorage.getItem('auth_token'),
      ...
      l =
        (i.success && i.data && 0 < i.data.length
          ? (updateUsersTable(i.data),
            updateStatCard('totalUsers', i.pagination?.total || i.data.length),
            updateUserPagination(i.pagination))
...
function updateArticlesTable(e) {
  ...
  t &&
    ...
    e.forEach(e => {
      ...
      e.is_scheduled &&
        (a += ` <span class="status scheduled" title="定时发布"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;margin-right:4px"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>${e.published_at ? formatDate(e.published_at) : '未设置'}</span>`);
...
function initAnalytics() {
  var e;
  ((e = document.getElementById('mostViewedLimit')) &&
    e.addEventListener('change', loadMostViewedArticles),
      ...
      t.addEventListener('click', async function () {
        ...
        if (
          ...
          try {
            ...
            var n = await (await fetch('/api/admin/passages?id=' + t, { headers: o })).json();
            if (n.success && n.data) {
              (await populateCategorySelect('editCategory', n.data.category || ''),
                (document.getElementById('editTitle').value = n.data.title || ''),
                (document.getElementById('editAuthor').value = n.data.author || '管理员'),
                (document.getElementById('editContent').value =
                  n.data.original_content || n.data.content || ''),
                (document.getElementById('editShowTitle').checked = !1 !== n.data.show_title),
                (document.getElementById('editCoverImage').value = n.data.cover_image || ''),
                (document.getElementById('editStatus').value = n.data.status || 'published'),
                (document.getElementById('editVisibility').value = n.data.visibility || 'public'));
          ...
          try {
            ...
            var o = await (await fetch('/api/admin/passages?id=' + t, { headers: n })).json();
            o.success && o.data
          ...
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var d = await (await fetch('/api/admin/users/' + t, { headers: n })).json();
            if (d.success && d.data) {
              const e = d.data;
          ...
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var s = await (await fetch('/api/admin/tags?id=' + t, { headers: n })).json();
            s.success && s.data
              ? ((document.getElementById('tagModalTitle').textContent = '编辑标签'),
                (document.getElementById('tagName').value = s.data.name || ''),
                (document.getElementById('tagDescription').value = s.data.description || ''),
                (document.getElementById('tagColor').value = s.data.color || '#007bff'),
                (document.getElementById('tagCategory').value = s.data.category_id || 0),
                (document.getElementById('tagSortOrder').value = s.data.sort_order || 0),
                (document.getElementById('tagEnabled').checked = s.data.is_enabled),
                (document.getElementById('tagForm').dataset.tagId = t),
                openModal('tagModal'))
              : showToast('获取标签详情失败：' + (s.message || '未知错误'), 'error');
          } catch (e) {
          ...
          try {
            const e = localStorage.getItem('auth_token'),
              n = { 'Content-Type': 'application/json' };
            e && (n.Authorization = 'Bearer ' + e);
            var r = await (await fetch('/api/admin/categories?id=' + t, { headers: n })).json();
            r.success && r.data
...
(uploadArea.addEventListener('click', () => {
  ...
  document.getElementById('uploadForm').addEventListener('submit', async function (e) {
    ...
    if (t) {
      ...
      if (n) {
        const g = document.querySelector('#uploadCategorySelector .category-option.selected'),
          h = g ? g.dataset.categoryName : '',
          y = [],
          p =
            (document.querySelectorAll('#uploadTagSelector .tag-option.selected').forEach(e => {
              y.push(e.dataset.tagName);
            }),
            this.querySelector('button[type="submit"]')),
          v = p.textContent;
        ((p.textContent = '上传中...'), (p.disabled = !0));
        try {
          var a = selectedFiles,
          ...
          if (0 !== a.length || o) {
            ...
            if (g && 0 !== g.trim().length) {
              var d = document.getElementById('uploadStatus').value,
                c = document.getElementById('uploadYear').value,
                s = document.getElementById('uploadMonth').value,
                r = document.getElementById('uploadDay').value;
              let o = null;
              if (c && s && r) {
                const e = parseInt(c),
                  t = parseInt(s),
                ...
                2020 <= e &&
                  ...
                  1 <= n &&
                  n <= 31 &&
                  (o = new Date(e, t - 1, n).toISOString());
              }
              var i = localStorage.getItem('auth_token'),
                l = { 'Content-Type': 'application/json' },
                m =
                  (i && (l.Authorization = 'Bearer ' + i),
                  { title: t, content: g, author: n, category: h, tags: y.join(','), status: d }),
                u =
                  (o && (m.created_at = o),
                  await (
                ...
                for (const c of a)
                  try {
                    const e = new FormData(),
                      ...
                      s = await a.json();
                    s.success ? n++ : (o++, d.push(c.name + ': ' + (s.message || '未知错误')));
                  } catch (e) {
                    (o++, d.push(c.name + ': ' + (e.message || '网络错误')));
                  }
                ((p.textContent = '创建成功!'), (p.style.background = 'rgba(255, 183, 122, 0.8)'));
                let c = '文章创建成功！';
                (0 < n && (c += ` 成功上传 ${n} 个附件`),
                  0 < o && (c += ` 失败 ${o} 个附件`),
                  setTimeout(() => {
                    (closeModal('uploadModal'),
                      (p.textContent = v),
                      (p.disabled = !1),
                      (p.style.background = 'rgba(255, 183, 122, 0.8)'),
                      this.reset(),
                      (uploadPreview.innerHTML = ''),
                      (selectedFiles = []),
                      fetchAdminData(),
                      showToast(c, 0 < o ? 'warning' : 'success'));
                  }, 2e3));
              } else
                ((p.textContent = v),
                  (p.disabled = !1),
                  showToast('创建失败：' + (u.message || '未知错误'), 'error'));
            } else
              (showToast('文章内容不能为空', 'warning'), (p.textContent = v), (p.disabled = !1));
          } else
  ...
  document.getElementById('categoryForm').addEventListener('submit', async function (e) {
    ...
    try {
      ...
      let c = {
        ...
        r = 'POST';
      t && ((s += '?id=' + t), (r = 'PUT'));
      var o = await (await fetch(s, { method: r, headers: d, body: JSON.stringify(c) })).json();
      o.success
        ? ((n.textContent = '保存成功!'),
  ...
  if (currentAction && currentItemId)
    if (currentAction.startsWith('batch-delete-'))
      ...
      try {
        ...
        else if ('delete-category' === currentAction)
          a = '/api/admin/categories?id=' + currentItemId;
        else if ('delete-tag' === currentAction) a = '/api/admin/tags/' + currentItemId;
        else if ('delete-main-card' === currentAction)
          a = '/api/about/main-cards/delete?id=' + currentItemId;
        else if ('delete-sub-card' === currentAction)
          a = '/api/about/sub-cards/delete?id=' + currentItemId;
        else if ('delete-attachment' === currentAction)
          a = '/api/admin/attachments/' + currentItemId;
