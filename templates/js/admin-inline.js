...
async function fetchCategories() {
  try {
    ...
    return a.success && a.data
      ? a.data
      : (console.error("获取分类列表失败:", a.message), []);
  } catch (e) {
...
async function fetchTags(e = null) {
  try {
    ...
    return c.success && c.data
      ? c.data
      : (console.error("获取标签列表失败:", c.message), []);
  } catch (e) {
...
async function fetchTags() {
  try {
    const e = localStorage.getItem("auth_token"),
      t = { "Content-Type": "application/json" };
    e && (t.Authorization = `Bearer ${e}`);
    const n = await fetch("/api/admin/tags", { headers: t }),
      a = await n.json();
    return a.success && a.data ? a.data : [];
  } catch (e) {
    return (console.error("获取标签列表失败:", e), []);
  }
}
async function populateTagSelector(e = []) {
  ...
  if (((t.innerHTML = ""), 0 === n.length))
    return void (t.innerHTML =
      '<div style="color: #999; font-size: 0.9em;">暂无标签</div>');
  n.forEach((n) => {
...
function updateSelectedTags() {
  const e = [];
  document
    .querySelectorAll("#editTagSelector .tag-option.selected")
    .forEach((t) => {
...
function updateSelectedTagsForNewArticle() {
  const e = [];
  document
    .querySelectorAll("#articleTagSelector .tag-option.selected")
    .forEach((t) => {
      e.push(t.dataset.tagName);
    });
...
async function populateCategorySelectorForUpload() {
  ...
  ((e.innerHTML = ""),
    0 !== t.length
      ...
      : (e.innerHTML =
          '<div style="color: #999; font-size: 0.9em;">暂无分类</div>'));
}
...
async function fetchAdminData(e = 1, t = 10, n = 1, a = 10, o = 1, c = 10) {
  try {
    ...
    const i = await fetch(`/api/admin/passages?page=${e}&limit=${t}`, {
        headers: d,
      }),
      r = await i.json();
    if (r.success && r.data)
      ...
      (showEmptyState("articlesTableBody", "暂无文章"),
        updateStatCard("totalArticles", 0),
        hidePagination());
    const l = await fetch(`/api/admin/users?page=${n}&limit=${a}`, {
        headers: d,
      }),
      u = await l.json();
    if (u.success && u.data)
      ...
      (showEmptyState("usersTableBody", "暂无用户", 7),
        updateStatCard("totalUsers", 0),
        hideUserPagination());
    const m = await fetch(`/api/admin/comments?page=${o}&limit=${c}`, {
        headers: d,
      }),
      g = await m.json();
    ...
    if ((console.log("统计数据响应:", y), y.success && y.data)) {
      ...
      if (e && void 0 !== y.data.yesterday_visits) {
        ...
        "up" === n
          ? ((e.className = "stat-change positive"),
            (e.textContent = `较昨日 +${t.toFixed(1)}%`))
          : "down" === n
            ? ((e.className = "stat-change negative"),
              (e.textContent = `较昨日 ${t.toFixed(1)}%`))
            : ((e.className = "stat-change neutral"),
              (e.textContent = "较昨日持平"));
      }
...
function generatePageButtons(e, t) {
  ...
  let a = Math.max(1, e - 2),
    o = Math.min(t, e + 2);
  (o - a < 4 &&
    (1 === a ? (o = Math.min(t, 5)) : o === t && (a = Math.max(1, t - 4))),
...
function addEllipsis(e) {
  ...
  ((t.className = "pagination-page ellipsis"),
    (t.textContent = "..."),
    e.appendChild(t));
}
...
function addUserPageButton(e, t, n = !1) {
  ...
  ((a.className = "pagination-page " + (n ? "active" : "")),
    ...
    a.addEventListener("click", () => {
      e !== currentUserPage &&
        fetchAdminData(currentPage, currentLimit, e, currentUserLimit);
    }),
    t.appendChild(a));
}
...
function updateCommentsPagination(e) {
  ...
  ((currentCommentsPage = t),
    ...
    (totalCommentsPages = Math.ceil(a / n)));
  ...
  t &&
    ...
    e.forEach((e) => {
      ...
      (n.querySelector(".article-checkbox").addEventListener("change", (e) => {
        (n.classList.toggle("selected", e.target.checked),
          updateBatchActionsBar());
      }),
        t.appendChild(n));
    }),
...
async function loadMostViewedArticles() {
  ...
  if (t) {
    ...
    try {
      const n = await fetch(
          `/api/admin/analytics?action=most-viewed&limit=${e}`,
...
async function loadViewSources() {
  ...
  if (t) {
    ...
    try {
      const n = await fetch(
          `/api/admin/analytics?action=view-sources&days=${e}`,
          { headers: getAuthHeaders() },
        ),
        a = await n.json();
      if (a.success && a.data) {
...
async function loadViewByCity() {
  ...
  if (t) {
    ...
    try {
      const n = await fetch(
          `/api/admin/analytics?action=view-by-city&days=${e}`,
          { headers: getAuthHeaders() },
        ),
        a = await n.json();
      if (a.success && a.data) {
...
async function loadViewByIP() {
  ...
  if (t) {
    ...
    try {
      const n = await fetch(
          `/api/admin/analytics?action=view-by-ip&days=${e}`,
          { headers: getAuthHeaders() },
        ),
        a = await n.json();
      if (a.success && a.data) {
...
async function loadViewTrend() {
  ...
  if (document.getElementById("viewTrendChart"))
    try {
      const t = await fetch(
          `/api/admin/analytics?action=view-trend&days=${e}`,
          { headers: getAuthHeaders() },
        ),
        n = await t.json();
      ...
      t.addEventListener("click", async function () {
        ...
        if (
          ...
          try {
            ...
            const a = await fetch(`/api/admin/passages?id=${t}`, {
                headers: n,
              }),
              o = await a.json();
            if (o.success && o.data) {
              (await populateCategorySelect(
                ...
                (document.getElementById("editShowTitle").checked =
                  !1 !== o.data.show_title),
          ...
          ((document.getElementById("uploadAttachmentArticleId").textContent =
            "#" + t),
            (document.getElementById(
              "uploadAttachmentArticleId",
            ).dataset.articleId = t),
            (document.getElementById("attachmentFile").value = ""),
            (document.getElementById("uploadAttachmentProgress").style.display =
              "none"),
            (document.getElementById("uploadAttachmentResult").style.display =
              "none"),
          ...
          try {
            ...
            const a = await fetch(`/api/admin/passages?id=${t}`, {
              ...
              o = await a.json();
            o.success && o.data
              ? ((document.getElementById("viewArticleId").textContent =
                  "#" + o.data.id),
                (document.getElementById("viewArticleTitle").textContent =
                  o.data.title || ""),
                (document.getElementById("viewArticleAuthor").textContent =
                  o.data.author || "管理员"),
                (document.getElementById("viewArticleStatus").textContent =
                  getStatusText(o.data.status || "published")),
                (document.getElementById("viewArticleDate").textContent =
                  o.data.created_at || ""),
            ...
            if (o.success && o.data) {
              ...
              ((document.getElementById("viewUserId").textContent = "#" + e.id),
                ...
                (document.getElementById("viewUserStatus").textContent =
                  e.status || "正常"),
                (document.getElementById("viewUserDate").textContent =
                  e.created_at || ""),
          ...
          e.length >= 6
            ? ((document.getElementById("viewCommentId").textContent =
                e[0].textContent),
              (document.getElementById("viewCommentContent").textContent =
                e[1].textContent),
              (document.getElementById("viewCommentArticle").textContent =
                e[2].textContent),
              (document.getElementById("viewCommentUser").textContent =
                e[3].textContent),
              (document.getElementById("viewCommentDate").textContent =
                e[4].textContent),
              (document.getElementById("viewCommentStatus").textContent =
                e[5].textContent.trim()),
            ...
            o.success && o.data
              ? ((document.getElementById("tagModalTitle").textContent =
                ...
                (document.getElementById("tagCategory").value =
                  o.data.category_id || 0),
                (document.getElementById("tagSortOrder").value =
                  o.data.sort_order || 0),
                (document.getElementById("tagEnabled").checked =
                  o.data.is_enabled),
                (document.getElementById("tagForm").dataset.tagId = t),
                openModal("tagModal"))
              : showToast(
                  "获取标签详情失败：" + (o.message || "未知错误"),
                  "error",
                );
          } catch (e) {
          ...
          try {
            const e = localStorage.getItem("auth_token"),
              n = { "Content-Type": "application/json" };
            e && (n.Authorization = `Bearer ${e}`);
            const a = await fetch(`/api/admin/categories?id=${t}`, {
                headers: n,
              }),
              o = await a.json();
            o.success && o.data
              ? ((document.getElementById("categoryModalTitle").textContent =
                ...
                (document.getElementById("categorySortOrder").value =
                  o.data.sort_order || 0),
                (document.getElementById("categoryEnabled").checked =
                  o.data.is_enabled),
                (document.getElementById("categoryForm").dataset.categoryId =
                  t),
                openModal("categoryModal"))
              : showToast(
                  "获取分类详情失败：" + (o.message || "未知错误"),
                  "error",
                );
          } catch (e) {
...
(mainTitle &&
  ...
  document
    .getElementById("uploadArticleBtn")
  ...
  document
    .getElementById("addUploadTagBtn")
  ...
  document
    .getElementById("addUploadCategoryBtn")
    .addEventListener("click", async function () {
...
(uploadArea.addEventListener("click", () => {
  ...
  document
    ...
    .addEventListener("submit", async function (e) {
      ...
      const a = document.querySelector(
          "#uploadCategorySelector .category-option.selected",
        ),
        o = a ? a.dataset.categoryName : "",
        c = [];
      document
        .querySelectorAll("#uploadTagSelector .tag-option.selected")
        .forEach((e) => {
      ...
      try {
        ...
        const d = await fetch(`/api/admin/passages?id=${t}`, {
            ...
            body: JSON.stringify({
              ...
              published_at: s,
              cover_image:
                document.getElementById("editCoverImage").value || void 0,
            }),
          }),
          i = await d.json();
        i.success
  ...
  document
    .getElementById("editIsScheduled")
    ...
    .addEventListener("submit", async function (e) {
      ...
      try {
        ...
        const c = {
          ...
          sort_order:
            parseInt(document.getElementById("categorySortOrder").value) || 0,
    ...
    .addEventListener("submit", async function (e) {
      ...
      try {
        ...
        const c = {
          ...
          category_id:
            parseInt(document.getElementById("tagCategory").value) || 0,
          sort_order:
            parseInt(document.getElementById("tagSortOrder").value) || 0,
    ...
    if (currentAction && currentItemId) {
      ...
      try {
        ...
        if ("delete" === currentAction)
          e = `/api/admin/passages?id=${currentItemId}`;
        else if ("delete-comment" === currentAction)
          e = `/api/admin/comments/${currentItemId}`;
        else if ("delete-user" === currentAction)
          e = `/api/admin/users/${currentItemId}`;
        else if ("delete-category" === currentAction)
          e = `/api/admin/categories?id=${currentItemId}`;
        else if ("delete-tag" === currentAction)
          e = `/api/admin/tags/${currentItemId}`;
          ...
          e = `/api/admin/attachments/${currentItemId}`;
          ...
          for (const a of e)
            try {
              const e = await fetch(`/api/admin/attachments/${a}`, {
                method: "DELETE",
              });
        ...
        a.success
          ? (closeModal("confirmModal"),
            ...
            "delete-category" === currentAction ||
            "delete-tag" === currentAction
              ? loadCategoriesAndTags()
