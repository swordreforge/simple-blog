!(function () {
        ...
        o.addEventListener('click', async () => {
          if (0 !== c.length) {
            ...
            for (const n of c) {
              ...
              try {
                const e = await fetch('/api/admin/attachments', { method: 'POST', body: t }),
                  a = await e.json();
  ...
  async function m() {
    var e;
    0 !== i.size &&
      ((window.currentAction = 'batch-delete-attachment'),
      (window.currentItemId = Array.from(i).join(',')),
      (e = `确定要删除选中的 ${i.size} 个附件吗？此操作不可恢复。`),
      (document.getElementById('confirmMessage').textContent = e),
    ...
    if (0 !== i.size) {
      ...
      for (const a of i)
        try {
          (
            await (
              await fetch('/api/admin/attachments/' + a, {
                method: 'PATCH',
      ...
      (0 < t && h(`成功设置 ${t} 个附件`, 'success'),
        0 < n && h(n + ' 个附件设置失败', 'error'),
        loadAttachments());
    }
  }
  function p() {
  ...
  function g() {
    var e;
    ((e = document.getElementById('adminShortcutsHelpBtn')) &&
      ...
      o.selectAll &&
        o.selectAll.addEventListener('change', e => {
        ...
        .then(e => {
          function t() {
            ...
            t &&
              ...
              (document.body.style.backgroundSize = e.background_size || 'cover'),
              (document.body.style.backgroundPosition = e.background_position || 'center'),
              (document.body.style.backgroundRepeat = e.background_repeat || 'no-repeat'),
              (document.body.style.backgroundAttachment = e.background_attachment || 'fixed'));
          }
  ...
  ((window.loadAttachments = async function () {
    try {
      const s = new URLSearchParams({ limit: n, offset: (t - 1) * n }),
        r = o.fileTypeFilter.value,
        m = o.visibilityFilter.value,
        u = o.passageFilter.value,
        p = o.searchInput.value.trim();
      (r && s.append('file_type', r),
        m && s.append('visibility', m),
        u && s.append('passage_id', u));
      var i = await (await fetch('/api/admin/attachments?' + s.toString())).json();
      if (i.success) {
        ((e = i.data || []),
          (a = i.total || 0),
          p &&
            ((e = e.filter(e => e.file_name.toLowerCase().includes(p.toLowerCase()))),
            (a = e.length)),
          c());
        {
          ...
          (e.forEach(e => {
            ...
            (o.totalSize.textContent = l(t)),
            (o.imageCount.textContent = n),
            (o.documentCount.textContent = i));
        }
        var d = Math.ceil(a / n);
        if (d <= 1) o.paginationContainer.style.display = 'none';
        else {
          ((o.paginationContainer.style.display = 'flex'),
            (o.paginationInfo.textContent = `显示 ${(t - 1) * n + 1}-${Math.min(t * n, a)} 条，共 ${a} 条`),
            (o.prevPageBtn.disabled = 1 === t),
            (o.nextPageBtn.disabled = t === d));
          let e = '',
            i = Math.max(1, t - Math.floor(2.5)),
            l = Math.min(d, i + 5 - 1);
          for (let n = l - i < 4 ? Math.max(1, l - 5 + 1) : i; n <= l; n++)
            e += `<button class="pagination-page ${n === t ? 'active' : ''}" \n                       data-page="${n}">${n}</button>`;
          ((o.paginationPages.innerHTML = e),
            document.querySelectorAll('#amPaginationPages .pagination-page').forEach(e => {
              e.addEventListener('click', () => {
      ...
      try {
        var a = await (
          await fetch('/api/admin/attachments/' + e, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ visibility: t, show_in_passage: n }),
          })
        ).json();
        a.success
          ? (h('保存成功', 'success'), closeEditModal(), await loadAttachments())
          : h('保存失败: ' + a.message, 'error');
      } catch (e) {
        h('保存失败', 'error');
      }
    }),
